...
> B100 单批：外部经验吸收（C20 六项+C21 三项+C22 三项，十二项落地）；agents_version **47** 不变（零 agent 模板变更，仅指纹确认性重登记 1329543f13fa）

### 新增与变更

- **C20 工序精化六项**：先大后小禁忌（防第一章写太满跑偏，逐层精化哲学负面约束版）+ 章末走到状态锚（细纲模板新字段+章后对照 landed/missed 联动）+ 信息差进攻性用法（主角误判/信息错位=期待引擎，防守红线外的主动设计视角）+ 卷篇幅配比参考（起 20/承 40/转 20/合 20，标非书证）+ 支线挂靠理由三分法（主线线索/塑造人物/铺垫设定，三者皆无不进表）+ 建纲心流四阶段注记（宏观俯视→降维代入→拉扯权衡→松弛留白，限设计侧翻译，禁止进运行时）
- **C21 原料扩展三项**：采风要素表五类扩七类（人物原型采风→原型-{主题}、时代质感采风→质感-{主题}，乌贼实证，无对标默认纳入/有对标跳过）+ 文字打磨路隔夜冷修时点（肘子节奏：隔夜后发布前；AI 等价功能=跨会话冷启动，审查隔离的时点版）
- **C22 正文参数补强三项**：爽点密度缺省参考（小爽点≥每 2000 字 1 个/间隔≤5000 字/大爽点数章铺垫，标无书证可覆盖——有对标/采风标尺时以其为准）+ 铺垫质量三问入倒推法（立靶/压到位/反差）+ 期待管理熟悉/意外配比维度（概念级，60/30/10 数字无出处弃用）
- **治理收口**：盲区 #16 退出码捕获纪律（管道不吞退出码/盯 CI 以 gh run list conclusion 为准）
...
- **采风接线断链**：源优先级表 v2（搜狗 web 限流实测→搜狗微信+360 搜索双 fallback/六内容源/禁用名单更新）+ spawn prompt 必含条款九条逐条搬入（治三期 3/3 agent 撞 Bing/Google 墙浪费 10+ 回合）
- **竞态条件**：轮 3 理想书评步前置检查（`ls 设定/采风-CF*.md` 文件系统证据绑定必经步——有产物先融合、无产物告知作者等或跳过）+ volume 采风补强同款前置检查
- **spawn 构造纪律入开发标准**：方法论操作约束须在 spawn prompt 逐条搬入不得摘要省略

## v2.5.2（2026-08-31）

> 实测二期五批（B88-B92）：心流与原创纪律+终审与告知闭环+合同统一与采风运营+网文作家工序萃取+挂载审计与回执——12 技能世代不变；agents_version **45**（部署物指纹五次重登记，世代不推移）
>
> **⚠️ 已部署项目**：重跑 `/moshu-setup` 刷新 CLAUDE.md（新增原创构建原则/告知三则/扁平路径）与全部流程文件，然后新开会话。

### 新增

- **原创双闸门（B88）**：参考三层合同（参数层直取/手法层归因通行/剧情层增量自证通行）+ 归因三问（作者为什么这么写→落读者心理→重新出发）部署至 CLAUDE.md 模板与 outline/采风流程——治「参考作品剧情照搬」
- **构建时假想读者（B88）**：单元拆分前峰值三问（本卷读者会截图发群的是哪三处→卷契约峰值时刻×3）+ 整合检验期待/释放排布表（>5 章干带构建时标红）
- **热梗手法学（B88）**：meme-craft.md（错位移植/吐槽位/梗作伏笔/保鲜原则/密度红线/双轨登记）+ 卷纲梗位标注 + write 侧接线
- **参考作品档案化（B88）**：一书一档 `设定/参考/{书名}.md`（速写确认后建轻档、采风回来增强）+ CF spawn 传档（researcher 只查增量）
- **终审层级化（B89）**：大纲终审（开书确认前，spawn evaluator structure 型+**长弧四检查面**：卷间跃迁契机/跨卷反转链/终局代价具体化/末卷收束节奏）+ 卷纲终审（产物包全量/遗留分类审计/缺口闭环）+ 遗留分类规则（跨卷兑现/执行分层/作者决策可留；本层不一致/悬空缺口必清）
- **定稿三查（B89）**：设定回写（卷纲新实体回写设定、新名过 naming-cards）/ 大纲影响（骨架级事实变更回改）/ 冻结声明（修订流引导+待定项便宜窗口）
- **入口导览+收官产物地图（B89）**：三 workflow 开场白块（步骤总览表；write 日更 LIGHT 版仅新批次首给）+ 确认/交接点产物地图 + CLAUDE.md 告知三则
- **微张力·段落级悬念（B91）**：hooks-paragraph 新增微张力节（每段末一个问号——追读力的毛细血管层，与章级钩子分层不合并）
- **黄金三章节拍精化（B91）**：BC-007 前三章段落级节拍（救猫咪 15 节拍表映射+题材卡优先弹性声明）
- **刀片计划（B91）**：skeleton-template 支线登记加刀片/离场列+character-construction 出场编排补刀片句——情绪武器负向调度
- **第一章专项（B91）**：多版本开头（2-3 版 300 字级作者选）+ 前 300 字专项打磨
- **灵感池入口（B91）**：idea-seed 加池模式（平时攒火种→开书过池筛活→进种子收敛）+ 轮 1 分流
- **心流模型序列维（B91）**：作家心流模型.md 十阶段×五要素合成总图（十环横切+十阶段序列两维）
- **挂载守卫脚本（B92）**：check-methodology-wiring.py 五级分级（MUST/LOAD/REF/MENTION/DEAD）+ DEAD blocking + CI 三处同步 + 回归测试——**「库存有接线无」的确定性终局收口**
- **加载回执机制（B92）**：三 workflow 每 Stage 开始报「📋 本步加载：{文件名}｜{一句要义}」——输入侧可见性，实测凭回执核对触发率

### 变更

- **路径合同扁平化（B90）**：`{书名}/正文/` 等嵌套 → 项目根直挂（**一文件夹一书**，作者裁定）——CLAUDE.md 模板/setup 状态四查/moshu 路由三处统一；部署弹窗三问改两问；多书切换保留为兼容
- **单元结构预览闸门（B89）**：拆完单元、写卡前 11 行结构总表停点确认——结构级返工窗口前移
- **打磨屏五选项→四选项（B89）**：📝 自改并入问句文本（适配 AskUserQuestion 上限）
- **采风协议运营强化（B90）**：回合预算（检索 ≤20 回合/先落骨架再补）+ Windows 环境节（临时目录 .caifeng-tmp/编码三坑）+ CDP 预检（spawn 前探测+征询，不自动启动）+ 国内可达源优先级表（搜狗→豆瓣书评/萌娘/百科API/平台问答；反爬名单；外网默认排除）+ 缺点与规避清单入六节模板正式节
- **跨技能引用侧名纪律（B90）**：ideal-review-template 断链修复 + 全仓裸链接排查 3 处清零 + 开发标准纪律行
- **令牌工单代写协议（B90）**：evaluator 评审报告由主会话代写工单走 review_tickets.py verify-token
- **三处弱挂载升级（B92）**：outline-causality DEAD→MUST / hooks-paragraph 索引级→LOAD / reader-psychology REF→LOAD——治 B86 萃取物接线落空
- **挂载强度分级规则（B92）**：开发标准新增——影响产出结构→必须 MUST / 影响质量→至少 LOAD / 纯工具→允许索引级 / 新方法论 DEAD=不准合入
- **五件套欲望-需求表述+峰值火种溯源（B91）**：Truby 欲望/需求框架入五件套说明；卷契约峰值时刻可标来源火种
- doc-budget 多次校准（五批增量吸收）；施工日志热文件+审核记录热文件保持热冷分离

## v2.5.1（2026-08-31）

> 实测一期修复四批（B84-B87）：打磨+模板+萃取+瘦身——12 技能世代不变；agents_version **45**

### 新增

- **大纲骨架模板 v2（B85）**：skeleton-template.md 四层十节——每卷骨架表从八列扩至**十列**（新增群像[谁入场/谁高光]+卷末钥匙[下一卷的具体问题]）+ **卷间驱动链**（前台问题→钥匙→下一卷问题全链连通检查，治「五卷五个故事」）+ **承诺兑现时点表**（模式转换点/卖点亮相点/群像入场点/终极露面点）+ **核心角色五件套**（新增动机字段——个人化理由，含对手）+ **因果前因注**（每卷标「因为上一卷的什么」）+ 对手梯队背后手笔列 + 终极阻碍介入点登记 + 刻度连续性行；check_outline 机检同步（旧版八列自动降级 candidate 兼容）
- **四项方法论萃取（B86）**：**金手指设计学**（三定则[能界/代价算法/成长同步]+四性+大奉式实例解剖+设计问句）/ **读者心理学**（开篇危机代入/爽点负债-偿付模型/代入机制/期待管理）/ **提纲动机链**（前提句北极星/动机→目标→障碍→冲突链/道德前提）/ **人物构建学**（人物宝典元素清单/roster 规模规划[核心团 5-8/跨卷反派 5-8/朝堂势力 8-15/单元轮换 20-40]/魅力原型学七型/出场编排表）——四文件均含消费接线（流程步加载点强制）
- **入口前置三查（B85）**：volume 入口加上游产物检查（设定/大纲缺→建议先走 /moshu-outline）/ review 入口加正文检查（空→建议先走 /moshu-write）
- **卷纲四字段（B85）**：单元卡承接钩/开启钩（单元间钥匙链）+ Stage 5 时点兑现核对 + 角色弧线信念轨迹（从信X到信Y）+ 跨卷敌手动机注

### 变更

- **台账改名「创作进度」（B84）**：设定/构建台账.md → 设定/创作进度.md + progress-template.md——日常功能是进度与状态，「台账」仅覆盖流水层
- **采风模板 v2（B84）**：头部核心发现节（产物自带要点表）/要素表消费去向列/要素 0 底色配方（模式占比+开局手法判定）/融合节角色澄清（agent 可预拟标注）/来源分组——治 query 框架污染
- **轮 3 预填路径（B84）**：作者可说「说不太准，按已有信息拟一版我来改」跳过子问题采集直接进草稿→改定
- **轮 1 参考速写确认（B84）**：AI 对每部参考作品给三行速写（底色+模式占比/开局手法 vs 全文主体/主角弧转变），作者确认后才组采风 query——治参考作品理解偏差污染下游
- **「例证≠需求」入宪（B84）**：宪法功能模仿原则补句——作者举例是原理解释不是内容需求，借原理不搬素材
- **步骤告知义务（B84）**：开发标准 §2.5 第四条——凡询问/停点必带一行「本步做什么·输入喂给什么下游」
- **交接措辞校正（B84）**：「转 /moshu-volume」→「建议你使用 /moshu-volume」（技能不调用技能，作者是调度者）+ volume 侧跨技能编号改业务名
- **全仓瘦身（B87）**：agent-references 死库存 7 个清理（零模板引用，src 保留）/ 施工日志审核记录热冷拆分（施工会话 345K→53K=**6.5 倍减负**）/ 规格归档（热目录 28→2）/ AGENTS.md 术语表分离（9780→7001，宪法原则 7 条完整保留）——注意力经济学落地
- **agents_version 45**：agent-references 部署物删除变更
...
- **写作上下文按需注入（B58）**：续写状态卡新增「点名清单」节（按下一章细纲出场顺序/涉及设定装配，列名级）+ 注入预算闸（默认 2500 非空白字符，双闸降级序：预算或硬限任一触发按出场顺序降级为列名+硬状态）+ **伏笔剧透锁**（信息差 reveal_chapter 未到揭示章时知情内容物理隔离）+ 细纲伏笔操作三态行（埋设/强化/回收+星级+情感迁移）
- **角色语声体系（B59）**：voice_samples 滚动 5 条语声锚（AI 挑选脚本管槽位）+ address_book 双向称谓域（A 称 B×B 称 A，称谓.md 派生+点名清单出场对≤8）+ **对话 DNA 五要素**（拆文→建卡→写作全链）+ doc-budget 覆盖面补全（review/analyze/setup/revision/import/deslop 六主 workflow 入册，CI 拦截覆盖全部热路径）
- **外科式修订护栏（B60）**：工单四可选键（problem 结构化/preserve 保留清单——填写权只作者与[需复核]转正/length_coefficient 字数系数/decision_points 编辑决策点）+ resolve「作者裁决：」前缀强制 + deslop 章级两轮上限/保留清单 grep 在位/事实保真自查
- **设定指纹与影响面（B61）**：design_fingerprints.py（record/diff/update，sha256+细纲涉及设定反查受影响章）+ 修订流 ③b 复查清单呈报（发起权在作者）
- **数值一致性（B62）**：progression 登记制数值域（direction up/down 登记锁定，回退无归因出候选/带归因明示）+ 数值正本优先级（state>细纲>正文回忆）+ 成稿长度候选（阈值 0.7/1.3 可覆盖）+ 数值正本两句入审查基准
- **卡文阻塞协议（B63）**：第 1 遍快写 3 种实质写法仍不满意→停笔上报（三版对比+卡点+三选项），禁跳过/注水/降标准
- **对标防撞对照（B65）**：Stage 5/修订流三维重合检查（人物功能位/桥段节拍/设定机制）+ **登记免责**（对标坐标/单元卡已登记项照列免计）+ 单源两维候选/多源共性降噪，协议下沉 cold-path

### 变更

- **追踪 schema 5→8**（reveal_chapter/voice_samples/address_book/progression 四新域，三次升级 migrate 带备份零丢失）+ 工单 SCHEMA 2 + **agents_version 39**
- **宪法 §3.4 升级**：schema 变更检查面三端→四副本+涟漪面七类全仓 grep（含 .json——B58 两轮 CI 红判因链入册）
- **审计法 v2.0**：产品双基线（3c 产品承诺符合性双向+前置基线新鲜度）+ 3a+ 旅程锚定走查——B66 整体审计首次完整应用（PRD 机制表 11/11 落实、产消零悬空、旅程零死机制、研究档台账 22 行零走样）
- **docs 三波瘦身 78→26 文件**：归档区/历史批次规格 B2-B57/旧审计报告/新开发者导读出库（历史均在 git）；README 地图重写
- **PRD 反哺**：新增 2.3.1 机制层 11 项表/五旅程嵌入新机制/v2.4.0 版本路线/3.14 数据契约显式 schema 8 四域三视图
- **术语表补录 8 条**（B67）：点名清单/伏笔剧透锁/语声锚/双向称谓/对话 DNA/进度登记/防撞对照/登记免责
- CI 修正四轮判因入册：工单跨分钟幂等（%H%M 分界重复工单根治）/schema 涟漪面/守卫 JSON 标记/CLI 帮助文本残留
...
- **审计修复（B31-B37）**：setup/build 全仓审计需修/候选落地——版本阈值散射闭合（bump 补 3 模式：小于/大于/不得用 v）、32 张文风卡 setup 副本登记 shared-assets、计数口径对齐（8 agents/8 项 verify）、CLAUDE.md 合并幂等规范化、verify 补 CLAUDE.md 标准节检查、全仓称谓统一（Phase/Step→Stage 含 B34 补齐）、write/review/scan 域修复（fanqie 采集降级崩溃、卷纲章节范围字段级断裂、工单 JSON 根字段文档、台账采风需求行、路由判定表对齐）
...
- **采风 agent 化**（批B21）：扩展 moshu-researcher 模板新增「采风研究」段（五类型：结构/角色/设定机制/情节/情绪；源七类：网文/名著/影视/漫画动漫/游戏/真实事件/历史事实——真实事件需改编脱敏）；**技能做壳 agent 做芯**（交互门+融合四步留主线，检索蒸馏归 agent，fallback 内联降级）；**CF 需求票据制**（台账登记/文件名自带/元数据记录——并发采风按 CF 对号入座）；产物六节结构（元数据含触发需求/来源清单含质量分级/要素表五类模板/专名清单/转译三问初答/融合与消费记录主线回写）；检索效率四件套（移植 gpt-researcher 模式：查询分解/两段式/源去重/引用强制）；机检新增"未消费采风"candidate；agents_version 31→32
- **舞台与规则设计法**（批B22）：回环（旧台新意义重访）/规则耦合（每台换玩法）/情感印记——升级台阶定义句扩舞台演进（选填）
- **暗线层次类型学**（批B22）：四类候选（外部阴谋/主角自我认知/主题真相/世界真相）选配——身世等降级为"主角自我认知层"的一种载体
- **情绪低压电路**（批B22）：plot-emotion-system 扩展——幽默/温情/悲后回温三种低压电流（衔接既有"日常小装逼"）+调剂密度问句；严肃题材豁免
- **配角高光列**（批B22）：支线登记表第 8 列（选填）+ 机检表头断言同步

### 变更

- **主题论证问句**入步 1 写法纪律（"这一卷对主张是辩护还是质问？"）
- **感情线接线**：workflow-build 步 2 引用既有 5 套感情线结构模板/层次设计（库存已足只缺消费）
- 采风要素表加舞台演进模式维度；agents_version 32→33（agent-references 内容变更）
- **溯源口径统一**：方法论节头"一书章节标题实证（溯源详见规格）"——产品文件专名零出现（防抄红线优先于溯源专名化，溯源细节由规格承载）

### 修复

- 修正 B19 研究盲区（researcher agent 初始快照已存在而未被发现）；采风教学例去作品化（观察 019）；caifeng-methods 要素表五类补齐（规格 v1.2 遗漏传导）

> 来源：test2-1 实测观察 016-023（含设计原则：通用性四问/填充测试+库存-接线区分/工具不问作者私人经历）。插件版本：build 1.3.0 / write 1.7.0 / research 1.1.0 / setup 1.5.0；agents_version 33。

## v2.2.0（2026-08-24）

> 构建管线五层齐备——从"靠大模型自己想"到"字段×方法论×机检×采风"四层解。test2-1 实测第二轮观察（013-020）修复落地。

### 新增

- **骨架行六要素与势力场**（批B16）：每卷表八列（一句话主角中心/主要对手+私人纠缠/危机赌注[三重死亡]/中点假胜假败/高潮定死/卷末跃迁）、写法纪律三条（附着元素/主语纪律）、对手梯队与势力场总览（网非链+可借力矛盾，势力间关系步1粗排步2回填）、常驻压力行（贯穿全书慢性危机，收尾卷与赌注合流）
- **三书结构逆向萃取**（批B17）：势力场设计（梯队分级/博弈矩阵/立场多样性+排队坐庄反面清单）、升级绑弧光（升阶=人格考验+弱绑定回退）、叙事装置（信息差装置/身份系统/悬念容器+复用≥5卷约束）三节进方法论（来源标注：大奉章节标题实证，另两书结构级推断）
- **大纲机检 check_outline.py**（批B18）：blocking 九项（结构完备/八列/行数/字数容差/占比/台阶算术/底牌/伏笔闭合）+candidate 四项（单链条提示/采风专名比对/常驻压力/反转覆盖）+版本兼容降级（旧结构项目零误伤）；CI 三处同步+11 用例回归——构建环确定性层从零到有
- **采风技能 moshu-research**（批B19，skills 11→12）：三类对象（结构/角色/设定机制）、跨媒介源（网文/动漫/影视/游戏）、转译深度三问（只改名=缝合不算转译）、防抄红线（专有名词零复用+来源占比披露+作者过目停靠）、build 三调用点解耦、B18 专名通配联动
- **支线·暗线·伏笔兑现三层**（批B20）：步1暗线设计节（层次表+读者感知/主角知晓错位——读者先知=悬念，主角先知=期待；信息差域构建期蓝本）、步3支线登记表（归属/交汇/占比≤25%/收束卷必填）、步4兑现方式列四选一+大伏笔铺垫链（跨≥3卷中间半揭/误导）、聚合视图附块

### 变更

- **题材公式降级为离线兜底**：genre-writing-formulas 头注定位四条（mo-shu 自定/适用短篇开篇弧/禁排长篇卷结构/势力场优先）+步3四处措辞——优先 /moshu-research 采风取活结构
- **方法论按节精读纪律**：带「节名」锚点的引用 grep/offset 定位后只读所需节，不整文件读入（热路径 token 负载估降 30-50%）
- **agents_version 30→31**：agent-references 内容更新（三书萃取三节+公式降级同步），重跑 /moshu-setup 生效

### 修复

- 采风教学例去作品化（咒力/咒术师组织→自造泛化例）；台账 v1 历史注记勘误；步4暗线推进接线（终审）

> 来源：`otherMaterials/testProject/test2-1/使用观察-v2.0.0.md` 观察 013-020。插件版本：build 1.2.0 / write 1.6.0 / setup 1.4.1。
