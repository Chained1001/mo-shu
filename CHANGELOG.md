...
> 实测驱动修缮——test2-1 全链路实测 12 条观察，7 条修复落地：步 0 四轮制 + 停靠/台账/交互模态升级 + 部署前置检查。

### 新增

- **交互模态规则**（批B15）：构建环四条对话形态选用纪律——封闭选择（候选 ≤4 可枚举）一律弹窗、开放采集对话式、同性质合批一屏、弹窗选项带"（推荐）+一句理由"
- **卷行总览聚合视图**（批B15）：停靠 2/3 屏首运行时 join 四源文件（大纲/角色弧线/单元卡/整合记录）按卷切九列——只展示不落盘（零第二真相），表注标源，缺失层标"待步 N 产出"
- **台账构建态状态机**（批B15）：`构建中 ｜ 定稿 ｜ 修订中 [日期]（stale N 待消化）｜ 开卷中 卷N`——修订裁决"改"通过即翻转并记 stale 基数，stale 消化清零+作者确认后回归定稿；开新卷进入/定稿同步翻转；进入规则汇报必含构建态
- **部署前置检查**：write/review/build 三 skill 入口检测 `.story-deployed`，缺失时不执行并提示先 /moshu-setup 新开会话（修复实测 UX 缺口：装了 skill 没跑 setup 直接写作）

### 变更

- **步 0 五问重构为四轮制**（批B14）：定调（开放对话：方向+参考书三个点+读者感觉）→ 定形（弹窗批：体量三数+目标平台+终局口径）→ 北极星（完美评述草稿改定）→ 定流程（仅精度档位一问）；删"你的优势"孤儿问（零消费）；终局口径措辞改"你要在动笔前设计好结局吗"；核心设定表补章数/卷数+终局口径槽位；**台账创建提前至四轮答完**（重启安全起点从步 0 收尾提前）
- **停靠主选择弹窗化+待定项分离**（批B15）：三停靠主选择改 AskUserQuestion（确认/调整/返回），待定项子问先弹、答完再弹主确认——主意图不再靠猜
- **台账维护纪律成文**（批B15）：停靠通过即回溯刷新状态列（不留"已通过仍显示待确认"）、待定项解决即出表、当前焦点每面更新
- **步 5 打磨记录落盘**（批B15）：LOCK 四查/字数配比/八节点坐标/卷级五问/完美评述终检写入整合记录.md——停靠 3 等待期关窗重启不丢检查结果
- **定稿收尾话术边界**（批B15）：定稿后只提示转 /moshu-write，不即兴建议 /moshu-setup 或修订/开新卷
- **设计期伏笔术语**（批B15）：计划埋/计划收/放弃——与正文期追踪术语（已埋/悬置）分离，防写作期状态错乱

### 修复

- 方法论声明三连问瘦身（批B14）：删"构建顺序"死配置（六步序列固定无分支消费）与重复档位问；三域规划深度不再问——默认档位表保留、作者口头可调

> 来源：`otherMaterials/testProject/test2-1/使用观察-v2.0.0.md` 观察 003-012；对标轻量化（观察 001）与深度拆解模板补句（观察 002）按作者裁决缓办。

## v2.0.0（2026-08-24）

> 构建迭代法 + 写作三遍法 + 知识策展 + 治理收口——全管线书驱化。

### 新增

- **构建迭代法 v3.0**（批B10）：moshu-build 从 Phase 1-3 重构为六步书驱构建（雪花法基底+网文适配+对标参照双路），每步五面（故事/人物/设定/情绪/对标），方法论书章节作为操作说明书，三停三档+完整交互脚本+浮现记录机制
- **写作三遍法**（批B12）：chapter-core B 段重构为快写/读者重读/技艺打磨三遍（帽子切换+上下文隔离铁律），四项读者检查（LOCK/冲突三层/弧线/辨识度）+五项技艺打磨（文风/节奏/钩子/对话/视角）
- **构建台账 v2**（批B3+B11）：六步三面状态追踪+浮现记录+六条流程规则+开新卷重置
- **对标节奏对照**（批B2）：卷纲坐标表固化（含事件锚点）+ volume-report 第④节确定性对照
- **知识策展管道**（批B5+B9+B13）：《小说的骨架》五条萃取（人物探索三件套/激发事件四问/完美评述）+消费接线+设定面情绪面补入
- **审计法 v1.3**（批B7+修订）：六步四半步 SOP（产消对账/引用内容抽检/异常降级链/安全面）+ AGENTS 自包含唯一宪法
- **3 新守卫**：check-reference-closure（传递引用可达性）/ check-route-write（路由残留语境判定）/ check-story-numbers 扩展（表行数断言）
- **impact_scan.py**：构建资产修订的影响分析（三清单确定性 grep）

### 修复

- B1b 路由残留 4 例修正（scan/analyze/setup/import 衔接表）+ B6 拆文产物定位声明 + scan-analyze/check_chapter_summary 读失败三分类 + 治理文档收口（执行总纲V2 归档、规格提级）

### 改进

- workflow-build 六步五面 + chapter-core 三遍法 + 台账v2 + 完美评述消费 + 细纲"本章涉及设定"字段 + 修订三遍法声明 + 教学书方法论内化
...
- **文风独立为 moshu-style 技能**：拆书不再产文风（Stage 1 表达层移除、Stage 6 改名「技法总结」产出 技法总结.md：情绪交替/可借鉴技巧/分层建议/不可模仿）；新技能 `/moshu-style` 从任意量原文产出 `文风库/文风.md`（句长/标点/对话技法/锚点，纯表达层）；写书文风召回唯一来源 = 文风库（缺失/不合规按交互提醒：生成/跳过记录/自写约束）；全仓 98 处文风引用同步（写书 9 文件 / setup 4 文件 / import 3 文件 / 契约守卫 3 处 / README 双语）。
- **全库逐 skill 审查修复**（5 个子代理并行 + 确定性扫描，0 断链 0 孤儿）：
  - 术语收敛：「分层建议/分层模仿建议」残留 6 处统一为拆书侧「分层学习路线」；`solo/direct` 确认是契约保留的跨 CLI 信号词（非 bug，恢复 8 处）。
  - 僵尸配置清理：对标原文分章残留（加载项/结构树/模板三处，见上）、刺猬猫参考表标注预留 + 读者段删除（4 平台范围）、import 章节分隔「优先级 2」指向不存在的步骤、`--per-volume` 不存在的参数提示。
  - 平台范围补全：scan 提问/全平台组合补七猫、晋江榜单表补全（月榜/完结金榜/新手金榜）。
  - 契约对齐：check_chapter_summary.py 花括号残留改**仅提示不判 FAIL**（与文档「不触发重跑」一致）+ 类型枚举权威改指 agent 模板；主题标签行数明确不纳入 4 条硬检查。
...
- **反 AI 10 条硬约束并入**（笔枢过渡写手，P0-1）：anti-ai-writing 14 模式扩至 20+——名词性独语句/句子可追问测试/最低信息量/禁直接命名情绪/拆物化结尾/禁群像反应/禁"了一下"句/禁破折号滥用/禁元叙事旁白/高频 AI 标记词每千字限额；三端（write/review/deslop + agent-references）共享同步，eval 样本更新。
- **群像反应确定性检测**（笔枢补学，三层分工）：check-ai-patterns.js 三副本同步新增「群像反应」模式（"全场震惊/所有人都…"），脚本做确定性、AI 不做统计。
- **审查 must_keep 保底**（引火 fix_tickets，P0-2）：review 裁决/修复意见显式带「不得破坏」项，防止修改破坏已认可内容。
- **命名卡 5 张**（V7 命名 14 张精选，P0-3）：`naming-cards.md` 冷路径——书名卖点承诺/章节名事件悬念/卷名阶段路标/角色名身份语系/代号绰号传播，outline-methods 路由接入。
- **钩子分类学补全**（V7 追读 11 类，P0-4）：hooks-chapter 新增支票/倒计时/渴望/认知/身份错位/选择六类，与既有 13 式章尾钩子合并。
- **章后对照**（笔枢 observer/arbiter 轻量版，P1-1）：workflow-daily 第 9 步——细纲落地核对（landed/missed/deviated）+ 认知边界核对（本章暴露信息 vs 细纲信息差），复用追踪事务基础设施。
- **本章设定包减法加载**（笔枢 trimmer 简化版，P1-2）：写前准备按细纲涉及角色/地点/线索过滤设定包，只加载本章要用的。
- **事实查证纪律**（引火查证协议，P1-3）：涉及现实知识/行业/年代事实先查证或标注存疑，禁凭记忆编造；写前准备 (g) 落地。
- **场景卡 6 张**（V7 场景 35 张精选，P1-4）：`scene-cards.md` 冷路径——团战/谈判对峙/真相揭露/重逢/宴会/审讯，共享资产同步。
- **认知边界三件套**（笔枢 info_boundary/info_voids 补学）：细纲新增「视角/信息差/认知边界」+「信息留白」字段，正文纪律 + 章后对照核对闭环；旧细纲无字段时从视角/信息差行与 INF 表推断，不阻塞存量。
- **写作技法总路由 + 技法卡 6 张**（V7 技法库学习）：`craft-cards.md`（信息/对白/情感/战斗/描写/视角类精选），writing-craft 总路由接入。
- **灵感种子增强**：idea-seed 补学引火 muse 的种子形态（beat-cards 同步增补）。

### 拆文确定性工程化

- **标准章节解析脚本** `chapter_boundary.py`（I-1）：章节/卷解析 + 连续性校验 + 边界表 + progress 骨架，替代 AI 每次手写解析（实测中文数字转换两轮 bug）；强制 UTF-8 输出（Windows GBK 控制台乱码）；边界表行号切片为唯一真值，消除双真值漂移。
- **章节摘要硬检查脚本化** `check_chapter_summary.py`（I-6/I-7）：4 条硬检查 + `--deep` 深度拆解 7 必含字段轻检查；类型/基调/主题标签枚举以脚本为**唯一权威**（流程审计 F1），AI 不再手写 grep 组合。
- **Stage 2 检查点机制**：防无限拆解、用户可控——检查点间隔选项 30/50/100/自定义试跑/一次跑完；间隔**不持久化**，续跑必补问；停靠点询问加「重新选间隔」，试跑与正式间隔可切换。
- **Stage 1 停靠点与检查点显式衔接**：防漏问/防重复问，断点模板表述统一。
- **部署完成提示最优路径引导**：扫榜→拆文→开书。

### 扫榜与部署

- **多榜单一次采集**：qidian-rank-scraper `--type` 支持逗号分隔多榜单；404 提示增强——移动端榜单下线时引导 CDP 重试。
- **分析脚本** `scan-analyze.js`：题材分布/指定题材条目/跨榜重复/完整简介，按条目计数（修正 AI grep 重复计数误报）。
- **输出目录约定**：`扫榜/{平台}{方向}_{YYYYMMDD}/`，明确不依赖拆文库；下游可达性约束 + topic-decision 同步。
- **收尾单步决策**：不问字数、拆文保持独立不耦合（用户反馈）。
- **部署一键脚本** `deploy.py`（三层分工落地）：deploy/verify 子命令、CLAUDE.md section 合并、版本默认值集中管理。

### 工程与守卫

- 共享资产 33→36 组（naming-cards/scene-cards/craft-cards 新增）；doc-budget 随新机制显式上调（全部带 why）。
- 改进清单 I-6/I-7 实现、I-8 轻拆设计否决登记（与扫榜/Stage 0-1 停靠重叠，写作端无消费点）。
- .gitignore 清理本地缓存目录。
...
> 三轮实测驱动的架构升级与方法论优化（借鉴 webnovel-writer v7 与 SparkArc Studio）：规则文本零裁剪、每步守卫验证 + 真实 Claude Code 无头走查（状态机/部署/开书/写前准备），全量回归 + CI 三平台验证。**版本变更**：7 个 skill 1.0.1→1.0.2，moshu-review 1.1.2→1.1.3，moshu-setup 1.2.8→1.2.9（agents_version 26 不变，agent 模板未改）。

### 架构升级（v7/SparkArc 学习落地）

- **分工原则宪法化**：moshu-write 核心方法第 6 条——脚本做确定性（机检/事务/守卫，能数的绝不让模型估）/ AI 做语义 / 作者做品味；登记为行为契约第 9 条（防删）。
- **单入口状态机**：moshu 路由新增「状态判定」9 序（未部署→无书→未完成拆文→开书→补纲→写章→卷末复盘→未完成审查），判定全来自文件系统不依赖会话记忆，与 session-start 近况注入叠加 = v7 next 心智。
- **作者控制点清单**：CLAUDE.md 模板「作者只负责四件事」——开书确认 / 细纲确认 / 审查裁决 / 卷复盘决策。
- **卷复盘机制**（填补真空缺）：`references/volume-review.md`——伏笔清账（三类）/ 卷摘要 / 下卷方向候选 / 契约修订候选；daily 批末检测卷末章触发；开新卷衔接卷复盘候选。
- **审查反馈闭环**（SparkArc Critic 反馈环）：review 的 S3/S4/advisory 写作建议 → `.moshu-review/review-log` → 补纲/日更消费（S1/S2 已由追踪事务闭环）。
- **质量预算阀门**（v7）：workflow-chapter 机检修复预算 = 2 轮统一阀门（字数/毒句式/退化/细纲照搬/标点共享），超限停止交作者防 token 失控。
- **审查作者裁决点**：full/lean 与 solo 双路径——全部接受 / 修改后接受 / 打回重写，未裁决不视作已处理。
- **备料默认化**：写前准备 explorer（context_load）已部署则优先，保留手动回退。

### 方法论优化（大纲/人物/两阶段）

- **大纲方法论总路由**：outline-methods「决策路由」升级为 16 行总入口（建纲任务→文件/节显式映射），workflow-setup 建纲引用，不凭记忆散取。
- **节拍卡集合**：`beat-cards.md` BC-001~012（压抑蓄力爆发/微反转补刀/情绪高压三段式/爽点换挡/卷终余波结算/冲突螺旋升级/黄金三章契约/爽压钩均衡/高潮构建/五折线/五阶段/期待感），细纲「本章结构公式」可引用节拍 ID——从既有方法论固化，零新理论。
- **人物主题卡 8 张**：镜像宿敌/伤口回避/价值自洽反派/导师旧债/守护底线/利益盟友/关系信息差/身份冲突（v7 人物库提炼，每张=适用/结构/注意）。
- **故事对象两阶段**：设定档案头部「状态：计划|已转正」——开书建档标计划，正文首现+追踪事务提交后转正；review 计划对象不得当事实引用（防"设计即事实"）。
- **方法论五项补强**（v7 落地，全链路衔接）：细纲时间锚点（→追踪 story_time）；世界观可选模块（货币经济/规则禁忌/资源限制/术语表）；信息差登记表 INF（→细纲引用→写前核对→review 维度）；创意约束表 10 条（→细纲契约风险→review 维度）；卷纲三类表（伏笔/悬念/感情线）+ tracking 类别约定（summary 前缀，零 schema 变更）+ 悬久阈值。
- **批末交叉核对**（实测抓到的产物矛盾根因）：卷纲↔细纲埋设章（执行口径优先）/ 时间口径 / 钩子衔接。

### 实测驱动修复（三轮真实测试暴露）

- **安装/部署**：README 补「安装后新开会话」提醒（skill 会话启动加载）；moshu-setup 部署交互 4 题→1-2 题（删除模板未用的悬空占位符）；符号链接安装 agent-references 同路径检测跳过自复制；Phase 1 自检一条命令。
- **扫榜环境**：agent-browser ENOENT 报错附一键修复指引（npm Windows 占用残留 .DELETE 的排查与重装命令）。
- **拆文工程**：进度表更新规范（精确 diff 防重复行）；字数口径硬规则（字符数权威、禁 wc -c 字节换算，实测 12MB 被误报 1200 万字）；字数落盘精度（边界表存精确字符数，防密度公式失真 ±25%）；**不物理分章**（边界表行号切片为唯一真值，防双真值漂移）。
- **细纲批次**：首批默认 5 章（原 10 章封顶，token 减半 + 细纲老化窗口减短）+ 细纲失效显式检查（角色退役/死亡 → grep 后续细纲标记复核）。

### 工程与守卫

- 行为契约 8→9 条（分工原则）；doc-budget 预算体系随新机制显式上调（含 why 说明）；shared-assets 新增 beat-cards 组（33 组 49 副本）；eval-prose-quality 入 CI；本地回归清单与 CI 完全对齐；CI 三平台 shared-file 一致性兜住 sync 漏提交（已修）。
