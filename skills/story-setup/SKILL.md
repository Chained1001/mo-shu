...
10. 检查 `AGENTS.md` 中的通用段（标题行含 `网文写作工具集（通用 Agent / Web AI）`）
   ...
   > 第 8-10 步只认各端**互斥**的标记。`skills/*/SKILL.md` 的 `metadata.openclaw` 不作 OpenClaw 信号：9 个 skill 全都带这个字段，而 OpenClaw / Reasonix / generic 三条 skills-only 路径部署出的 `skills/` 长得一样，用它判定会把后两者一律误认成 OpenClaw。`.agents/skills/` 同理由 Codex 与 Reasonix 共用，也不单独作准。三端真正的分辨点是各自 `AGENTS.md` 模板的标题行。
...
| `skills/story-setup/references/opencode/commands/` | `.opencode/commands/` | story-setup managed | replace | 9 command files exist | target_cli 含 opencode |
...
| repository `skills/{browser-cdp,story*}/` | `.zcode/skills/{browser-cdp,story*}/` | story-setup managed for known skill names | replace known skill dirs only | 9 `SKILL.md` files exist and satisfy ZCode frontmatter limits | target_cli 含 zcode |
| `skills/story-setup/references/zcode/commands/` | `.zcode/commands/` | story-setup managed for known command names | replace known command files only | 9 commands have valid names/frontmatter | target_cli 含 zcode |
...
| repository `skills/{browser-cdp,story*}/` | `skills/{browser-cdp,story*}/` | story-setup managed for known skill names | replace known skill dirs only | 9 `SKILL.md` files exist; OpenClaw-compatible frontmatter | target_cli 含 openclaw 或 generic 或 reasonix |
...
1. 复制仓库当前 `skills/` 下 9 个包含 `SKILL.md` 的目录到 `.zcode/skills/{skill-name}/`；仅替换这些已知目录，保留用户其他 Skills。
2. 复制 `references/zcode/commands/*.md` 到 `.zcode/commands/`；仅替换 9 个同名命令，保留用户其他 Commands。
...
1. 读取仓库当前 `skills/` 下所有包含 `SKILL.md` 的 story skill 目录（9 个：`browser-cdp` 与 `story*`）。
...
1. 读取仓库当前 `skills/` 下所有包含 `SKILL.md` 的 story skill 目录（9 个：`browser-cdp` 与 `story*`）到目标项目 `skills/{skill-name}/`；仅替换这些 story-setup 管理的已知 skill 目录，保留用户其他目录。
...
1. 复制仓库当前 `skills/` 下所有包含 `SKILL.md` 的 story skill 目录（9 个：`browser-cdp` 与 `story*`）到目标项目 `skills/{skill-name}/`；仅替换这些 story-setup 管理的已知 skill 目录，保留用户其他目录。
...
7. 验证 opencode 部署（仅当 target_cli 含 opencode 时）：
    ...
    - 检查 `.opencode/plugins/lib/story_hook_core.js` 存在且 `node --check` 通过（story-hooks.ts import 之，与 `.zcode` 副本字节一致的共享写正文守卫核；置于 `lib/` 子目录以避开 OpenCode 单层 `.opencode/plugins/*.js` 插件自动发现）
     - 检查 `.opencode/commands/` 下的 9 个 command 文件是否存在
...
9. 验证 ZCode 部署（仅当 target_cli 含 zcode 时）：
    ...
    - 检查 `.zcode/skills/` 下 9 个 Skills 与 `.zcode/commands/` 下 9 个 Commands，验证 frontmatter 和命名
...
11. 验证通用 Web AI / 其他 Agent 部署（仅当 target_cli 含 generic 时）：
    ...
    - 检查 `skills/` 下 9 个 story skill 目录存在，且每个 `SKILL.md` 可读
    - 检查 `skills/story-setup/references/agent-references/` 下 reference 文件完整且数量与源目录一致
...
| references/zcode/ | ZCode AGENTS、9 Commands、workspace config patch 与严格 JSON Hook runner |
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 部署完成，开始写作 | story-long-write | `/story-long-write` |
