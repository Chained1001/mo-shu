...
const { ab, sleep, evalJSONBase64, safeStr, scrollLoad, getArg, localDateStamp, localTimestamp, runCli } = require("./cdp-utils");
...
function main() {
  if (CHANNEL !== "all" && !CHANNELS.some((c) => c.id === CHANNEL)) {
    throw new Error(`未知 --channel: ${CHANNEL}（可选 male/female/all）`);
  }
  ...
  for (const ch of channels) {
    ...
    if (!content) {
      ...
      partialReasons.push(`${chInfo ? chInfo.label : ch}: no usable data`);
      continue;
    }
