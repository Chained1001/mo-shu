...
- **采样原则（三维）**：
  ...
  3. **描写类型覆盖**——选取章节尽量覆盖不同类型（**对话推进 / 动作打斗 / 情感心理 / 景物开篇**），避免 3 章全是同一类型导致文风偏科；从章节标题与开头判断类型（如"牢狱之灾"=景物+心理开篇、"我要包场"=对话交易、"潜龙城"=场景+对话）。选章后逐章标注主导描写类型，写入生成记录——**文风应能回答"作者怎么写对话/景物/动作/心理"四问**
