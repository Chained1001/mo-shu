...
- **采样原则：完整章节优先**——有完整章节（本地文件）时取 **2-3 个完整章节**（分散在开头/中间/后期，每章全文；网文一章 2000-4000 字）。前 N 字采样有"开篇偏差"（开篇 1000 字多为环境/设定密集区，对话样本稀少、形态单一——实测《大奉打更人》第 1 章前 1000 字几乎零对话、第 100 章前 1000 字全是传书格式），完整章节才能覆盖"开头铺垫→中段对话→结尾钩子"的完整形态
