...
- **采样原则（档位制——样本覆盖面直接决定文风质量与写作效果）**：
  - **标准档 5-6 章（推荐，正式学习文风用）**：完整章节全文，综合覆盖——**基础四类**（对话推进 / 动作打斗 / 景物开篇 / 情感心理）+ **本书高频场景**（智斗推理/日常生活/群像多人/设定系统，按题材从章节标题关键词分布判断）+ **位置分散**（开头/中间/后期各 2 章）。5-6 章的样本量（2-3 万字）能让锚点覆盖四类叙事功能且每种描写类型都有真实素材，文风文件对写作的指导覆盖面最广
  - **最小档 2-3 章（快速评估用）**：判断"这本书值不值得学/拆"、或时间受限时使用——基础四类保底 + 按书挑 1-2 类高频；**覆盖缺口必须在生成记录诚实标注**（如"动作类样本不足，未覆盖"），不编造
...
- **选章通用规则**：
  ...
  - 逐章标注主导场景类型，写入生成记录——**文风应能回答"作者怎么写对话/景物/动作/心理"四问 + 本书高频场景写法**
