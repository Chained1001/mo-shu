...
- **采样原则（三维）**：
  1. **完整章节优先**——有完整章节（本地文件）时取 **2-3 个完整章节**（网文一章 2000-4000 字）。前 N 字采样有"开篇偏差"（开篇 1000 字多为环境/设定密集区，对话样本稀少、形态单一——实测《大奉打更人》第 1 章前 1000 字几乎零对话、第 100 章前 1000 字全是传书格式）
  2. **位置分散**——章节取开头/中间/后期各一（覆盖全书文风演变，后期章节句长/成熟度可能不同）
  3. **场景类型覆盖**——基础四类 + 本书高频场景：
     - **基础四类**（保证"作者怎么写对话/动作/景物/心理"有样本）：对话推进 / 动作打斗（含战斗）/ 景物开篇 / 情感心理
     - **按题材补本书高频场景**（2-3 章覆盖不了 8 类，按书挑）：从章节标题关键词分布判断本书高频场景——智斗推理（案/查/秘/审，探案权谋系统文）、日常生活（吃/家/闲，种田日常文）、群像多人（会/宴/场）、设定系统（面板/系统/规则）——基础四类之外至少覆盖本书高频 1-2 类
     - **选章后快速验证**：读所选章节开头 2-3 段确认实际类型——标题判断不可靠（实测《大奉打更人》第 183 章标题"血光"但内容是传书群聊+日常，非打斗章），不符则换章或接受覆盖缺口并在生成记录注明。选章后逐章标注主导场景类型，写入生成记录——**文风应能回答"作者怎么写对话/景物/动作/心理"四问 + 本书高频场景写法**；某类型覆盖不足时诚实标注（如"动作类样本不足，未覆盖"），不编造
...
- **生成记录**：来源（本地文件路径/粘贴）、抽样位置、**抽样章节及主导描写类型**（如"第1章牢狱之灾=景物+心理开篇 / 第100章=对话交易 / 第500章=场景+对话"）、生成时间、`文风可用：是/否`
