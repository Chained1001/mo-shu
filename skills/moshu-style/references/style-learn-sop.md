...
- **描写类型覆盖**：锚点尽量覆盖不同类型——对话段 / 景物段 / 动作段 / 心理段各选代表（与采样三维呼应，保证 few-shot 能示范"作者怎么写对话/景物/动作/心理"）；样本不足的类型写明跳过，不编造
- 每段 300-500 字；标注**基调 + 描写类型**（如"基调：紧张；类型：对话"）；基调按样本内容判断（紧张/悲伤/轻松/热血等）
