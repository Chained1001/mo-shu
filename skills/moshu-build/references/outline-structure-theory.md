...
1. **选段（以剧情单元为单位）**：在主对标里圈出本卷要对标的一级结构序列——选段单位就是拆文产出的剧情单元（`剧情/{剧情标题}.md`）。检索顺序：先读主对标 `剧情/README.md`「剧情单元清单」（有清单则一书一文件，按「类型 / 桥段标签」直接命中同类）；无清单则在 `剧情/*.md` 里按「类型」行（第一键，必填枚举）+「桥段标签 / 套路框架位置」行（第二键）圈同类；仍无命中回退按章节范围圈段，全程不阻塞。读取必备权威文件 `剧情/节奏.md`（爆发密度 / 爽点循环）；缺失时先修复或重跑拆文 Stage 3，不从摘要猜测节奏。本卷对标几条一级结构 ≈ 本卷字数 ÷ 1.5-3 万。
