...
| 派生视图 | `上下文.md`、`角色状态/{角色名}.md`、`伏笔.md`、`信息差.md`、`时间线/作者真相.md`、`时间线/读者已知.md` | 完全从 `_tracking-state.json` 生成；禁止手改，不作为程序输入 |
...
本工具不解析旧 `_tracking-meta.json`、`时间线/事件库.json` 或更早追踪结构，不提供语义兼容层。`init` 遇到这类旧文件时，先把它们按受管清单移入 `追踪/_旧追踪存档/`，再在原地建当前协议：旧内容留给作者查阅，不参与解析，当前状态完全以 init 输入为准。校验失败的 `init` 不移动任何文件。`commit` 与 `check` 仍直接拒绝旧结构——它们只在已建协议的项目上运行。

## 初始化事务

新书从第 0 章初始化。`moshu-import` 导入已有小说时把最后完整章写入 `last_chapter=N`；第 1..N 章不伪造日更记录，常规续写从 N+1 章开始。
...
- **线索类别约定（伏笔/悬念/感情线共用 foreshadow 字段，零 schema 变更）**：`foreshadow` 条目的 `summary` 用前缀标注类别——`悬念：{内容}`、`感情线：{内容}`、`债务：A→B`（叙事债务，已有约定）；无前缀默认伏笔。悬久阈值按类别区分：伏笔/悬念紧（约 10 章量级），感情线宽（约 30 章量级）——感情线条目 `planned_resolution_chapter` 按宽阈值规划，不触发紧阈值误报。三类在 `伏笔.md` 当前行与续写状态卡「活跃伏笔」栏统一呈现（按前缀可区分）。
