...
| 写都市高武，需要情节素材 | [genre-writing-formulas.md](genre-writing-formulas.md)「都市高武情节模板」 |
...
> 扫榜/拆书操作与素材提取的详细方法（平台榜单选择/拆书维度/素材提取纪律）与 build 侧采风环节职能相邻——此处仅保留 5 行摘要：

- 扫榜：按平台看不同榜单（番茄阅读榜/新书榜，起点三江/推荐位）；金榜是成熟作品适合抽结构标准
- 优先看同平台、同题材、非头部光环作品——结构样本更可复用
- 拆书前先明确要拆的维度，不带个人好恶，每次只拆一个维度（人设/节奏/对比手法/悬念等）
- 带着具体问题去拆（如"怎么冲突拉扯情绪"→找十段冲突情节分析）
- 拆书记录标注具体维度，以写作功能位提取（热点元素/冲突结构/情绪链条/节奏编排），不脱离题材
...
> 消歧：本节的「X级结构」指**平台订阅表现视角**（追读稳定 vs 订阅波动/阶梯型流失）；结构层级语境的「X级结构」见 outline-structure-theory.md、方法分级语境的见 outline-methods.md「故事结构分级」——同名不同义，按语境取用。
...
> 本节内容与 [outline-rhythm.md](outline-rhythm.md)「改编法」重复（通过矛盾找目标/对照法拆改/逐项验收），已收敛至该处——此处不再重复维护。
...
> 中后期写法（追订下降应对/围绕结局设计爆点）属 write 侧商业留存方法（中后期追订两大原因/围绕结局设计中后期爆点），已收敛至该处——此处仅保留卡点倒推 5 行摘要：

- 先确定卡点（上架高潮场景）→ 从卡点倒推开头剧情，一步步严丝合缝推导
- 三万字内无关卡点的装逼打脸一个字不要写
- 核心反派要围绕卡点设计
- 中后期两大原则（一年推进一次主线/着眼于一天后的事）见 commercial-core-methods 中后期节
...
> 通用检查项均对应本文档正文节（金手指拆分/题材边界/小循环四步/阵营模式/爱情线/卡点倒推）——按节回查；以下为交叉特有项：

- [ ] 后期金手指仍有多维度成长（词条+功能+品质），不会只剩品质提升
- [ ] 升级线已建立（境界体系或等效框架）；社会环境有足够的社会关系支撑（不只是主角+敌人）
- [ ] 金手指核心卖点循环在题材边界内，未偏离；读者冲什么进来的，后续持续给什么
- [ ] 小循环中铺垫了大循环的期待；同人用一级结构（单元剧），原创按节奏递进模式
- [ ] 情绪波浪式推进（避免一路压抑或一路爽到底）
- [ ] 改编后的剧情与原型已有本质差异（世界观/人物/金手指不同）；对标书为同平台同题材可复用样本

## 叙事装置（逆向萃取——结构层，非原书方法论；一书章节标题实证，另两书结构级推断（溯源详见批B17 规格））

> 与 B16「常驻压力」行及总反转层级衔接：叙事装置是常驻张力的载体，悬念容器是人生叙事级伏笔的收束点。

### 三类装置

1. **信息差装置**：主角知道的秘密随剧情重构——定期聚会/信道/日记体等载体，让「谁知道什么」持续变动（读者与角色的信息差是张力来源）。
2. **身份系统**：多重身份的暴露危机=常驻张力（身份一旦揭穿，前期积累全部翻转）。
3. **悬念容器**：开篇埋、终局开的「箱子」——人生叙事级伏笔对象，收束时改写主角自我认知（不是普通物件的回收）。

### 设计约束

- 每书最多 1 个主装置 + 1 个悬念容器（装置过载=读者疲劳）；
- 装置必须复用 ≥5 卷（一次性的不是装置是桥段）。