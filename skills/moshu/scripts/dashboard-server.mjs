...
function shouldIgnoreDirectory(name) {
  return IGNORED_DIRECTORIES.has(name) || (name.startsWith(".") && name !== ".moshu");
}
...
export async function resolveWorkspacePath(root, requestedPath, options = {}) {
  ...
  return { absolutePath: candidate, resolvedPath: resolvedFile, realRoot, info };
}
...
async function readWorkspaceFile(root, requestedPath) {
  ...
  if (content.includes("\uFFFD")) {
    throw new DashboardError(
      ...
      "文件不是有效的 UTF-8 文本（可能为 GBK 等编码）。为避免保存时损坏原文件，请先转码为 UTF-8 后再在 Dashboard 中编辑。",
    );
  }
  return {
...
async function saveWorkspaceFile(root, payload) {
  ...
  if (payload.content.includes("\uFFFD")) {
    throw new DashboardError(
      400,
      "invalid_content",
      "保存内容包含无法还原的替换字符（U+FFFD），通常是文件并非 UTF-8 编码所致，拒绝写入以免损坏原文件。",
    );
  }
  ...
  return withSerializedFileMutation(initial.resolvedPath, async () => {
    let current;
    try {
