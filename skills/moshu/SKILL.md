...
| 去AI味 | 去AI味、太 AI、去味 | /moshu-write 改稿·文字打磨路（B95 deslop 收编） |
...
| 2 | 无 `正文/`、`大纲/`、`追踪/` 任一目录（项目根直查） | 开书引导（或先扫榜/拆文/选题） |
...
- **无项目目录**（项目根无 `追踪/` 或 `设定/` 或 `大纲/` 任一目录）：用户要写作 → 下一步先 `/moshu-setup`；要扫榜/拆文 → 直接路由。
- **已有项目**：检查 `.story-deployed` 标记，未部署则先 `/moshu-setup`。

## 多书切换

> **主流用法=一文件夹一书（作者裁定 2026-08-31）；多书切换为兼容保留**——开新书建议开新文件夹。
