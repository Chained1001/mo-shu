# scan-workflow.md：moshu-scan 完整扫榜流程

本文件承载 moshu-scan 的完整扫榜流程（Phase 2-5：数据来源、数据分析、报告模板与选题决策）。SKILL.md 只保留定位、核心哲学与 Phase 索引；进入 Phase 2 后按本文件对应节执行。
...
**输出规范**：详见 [scan-output-format.md](scan-output-format.md)，包含各平台字段定义、输出模板。
...
每完成一个榜单的采集，立即执行以下检查。发现问题当场修复，不留给后续分析。详细规则见 [scan-output-format.md](scan-output-format.md)「数据清洗与字段约束」。
...
把扫榜结果变成能直接用的选题建议，产出 `选题决策.md`。完整方法（选题四步 + 可行性判断 + 输出模板）见 [topic-decision.md](topic-decision.md)。
