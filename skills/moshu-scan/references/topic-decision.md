...
2. **收尾交互收敛成单步决策**：展示 3 个选题（对比表：方向/核心卖点/差异化/可行性/验证动作）后，只问一个决策——「你倾向哪个选题？或都不满意？」
...
3. **拆文保持独立，不自动衔接**：提示一句「想坐实'能爆的原因'可自行 `/moshu-analyze` 拆 {对标书}，拆完回填」即可——拆不拆、何时拆由用户决定，选定选题不代表进入拆文。
4. 告知用户路径，并说明下一步：「开书时如果小说项目就建在这个目录里、或和这个目录同级（共用一个上级目录），`/moshu-write` 会自动找到并让你确认；隔得更远就把 `选题决策.md` 复制到小说项目根目录，或开书时把路径粘给它。」
5. 后续衔接（本文件不执行，仅说明）：拆文在汇总报告产出后回填对应选题的"能爆的原因"；写作 Phase 1 读取 `选题决策.md` 作为开书起点。
