...
加载 [anti-ai-writing.md](anti-ai-writing.md) 的「系统性去AI三遍法」获取完整流程。三遍法与本 skill 的关系（覆盖关系，不是 1:1 映射）：
...
加载 [banned-words.md](banned-words.md)，对照禁用词表逐项检查。
...
- **保留清单遵守**：工单 `preserve` 列出的句段/事实**逐条不动**——整改前后各 grep 一遍引文确认在位（流程纪律，不脚本化，D3 定值）；reviewer/deslop 不得自填 preserve（填写权只在作者与 `[需复核]` 转正）。
...
> **review 整改路径引用**：moshu-review 工单整改执行同一纪律，见本节（反模式 #4：不复制两份）。
