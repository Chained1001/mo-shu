 ...
 * 判定保留的只由主会话补进细纲锚句：子代理不改大纲细纲（见 moshu-narrative-writer「不自行改纲」），
 ...
 *
 * 由 moshu-narrative-writer 落盘后自查、主会话收尾复扫时调用（两侧同一份实现，口径一致）。
 ...
 *
 * 退出码：0 = 干净或无法判定（缺细纲/非分章正文）；1 = 有重合待复核；2 = 不可读文件/参数错误/内部异常（fail-loud，不静默判干净）。
 * 无发现时完全静默，不污染上下文。
 */
...
function main() {
  ...
  for (let k = 0; k < argv.length; k++) {
    if (argv[k] === '--outline') {
      if (k + 1 >= argv.length || argv[k + 1].startsWith('--')) {
        process.stderr.write('错误: --outline 缺少参数\n用法: node check-outline-copy.js [--outline <细纲路径>] <正文路径...>\n')
        return 2
      }
      explicitOutline = argv[++k]
...
try {
  ...
  process.stderr.write(`check-outline-copy: 内部错误: ${err && err.message ? err.message : err}\n`)
  process.exit(2)
}
