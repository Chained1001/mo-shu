...
| 世界时间推进 | 暗线/势力动态预期 |
|------|------|
| 1-2 天 | 大部分维持不变，仅紧急事务可能推进 |
| 3-7 天 | 快节奏暗线明显推进，慢节奏小幅推进 |
| 8-30 天 | 所有暗线应有推进，可能出现新世界事件 |
| 30+ 天 | 显著格局变化，可能势力洗牌 |

> 暗线 = 镜头外持续推进的伏笔、势力动向与未在正文展开的事件。mo-shu 不单独建暗线追踪：暗线记录在 `伏笔.md`（已埋未回收）与 `timeline_events`（世界事件/势力变化）中，不另设文件。

按本章 `story_time` 推进量核对暗线是否该有进展；长时间跳过后暗线全无变化，是世界观"冻住"的信号（写手补事件或显式写"无进展"）。

### 叙事债务（角色间承诺）

角色间的承诺、悬置因果与信息差（A 答应 B 的事还没兑现、A 不知道但该知道的事）是"角色欠角色"的债务，**区别于伏笔**（伏笔是"读者想知道答案"，债务是"角色欠角色"）。记法：

- 有明确回收预期的角色承诺 → 记入 `foreshadow`（summary 标注"债务：A→B"），随 `foreshadow_changes` 更新当前行；
- 跨章悬置、暂无回收章 → 写入 `continuity_risks`，避免债务随会话丢失；
- 判断依据：读者想知道答案 → hook；角色欠角色 → debt。
