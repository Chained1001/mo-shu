...
Stage 3-L 长篇结构迁移的详细映射规则和模板。将 `拆文库/{导入书名}/` 分析结果转换为 `{导入书名}/` 长篇项目结构。
...
| `剧情/节奏.md` | `设定/基本设定.md`（节奏摘要；B102 前旧名题材定位.md） | 提炼已写部分的既有节奏，保留为本书导入基线；不复制到 `对标/` |
| `剧情/情绪模块.md` | `设定/基本设定.md`（情绪摘要） | 提炼已写部分的读者需求与情绪引擎；不复制到 `对标/` |
...
当源为单个大文件时，按以下分隔符切分（与 `import-workflow.md`「Stage 1-3：输入方式识别」复用同一识别表）：
...
当前 `moshu-analyze` 已输出主题化目录，导入阶段只做 pass-through，不再解析或拆分扁平 `世界观.md`。
...
`力量体系.md`、`地理.md` 或小势力资料不足 200 字时，上游会将其并入 `背景设定.md`，因此这些独立文件可省略。如缺少 `背景设定.md`，或当前内容指向独立力量体系却未产出对应文件，停止导入并提示重跑 `moshu-analyze` Stage 2-5。
...
（导入反推只填有证据的字段，未知写 `[待补充]`、不杜撰；后续补纲/改纲时按 moshu-write 技能的「剧情单元卡」完整字段模板升级。）
...
从拆文报告中提取核心发现，生成 `设定/基本设定.md`（B102 前旧名题材定位.md）。
...
## 基本信息
- 题材类型：{玄幻/都市/系统/...}
- 目标平台：{Stage 1 向用户采集的目标平台；无则从拆文报告提取，仍无填 [待补充]。moshu-review 据此选平台 rubric}
- 核心梗：{一句话卖点}
- 微创新点：{与同类题材的差异}
...
## 对标登记（canonical registry，可选）
主对标书: {对标书名}  # 最多 1 本；必须是独立外部参考作品

## 对标分析（派生概要）
> 完整对标数据见 `对标/` 目录；上方 registry 是权威清单。本表仅做快速概览，不可替代 `主对标书`。
...
- 读者需求 / 情绪引擎 → 从 `剧情/情绪模块.md` 提取；缺失时停止导入并给出重跑 Stage 2-4 起的修复动作
...
| `拆文库/{对标书名}/技法总结.md` | `{项目}/对标/{对标书名}/技法总结.md` | 作者参考（同拆文报告待遇） |
