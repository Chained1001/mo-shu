...
> 仅用于 `moshu-import` 导入长篇。输出不是一份不断追加的角色历史，而是初始化事务里的 `character_snapshots`；`tracking_commit.py init` 会为每个核心角色生成 `追踪/角色状态/{角色名}.md`。
...
- 分批导入：快照只表示本次已导入范围的当前状态；扩大导入范围时对新增已写章节逐章构造 append 事务（章号连续）并 `commit`，不在旧快照上追加历史段落，也不要重跑 `init`（`init` 仅在追踪状态不存在时可用，绝不覆盖既有项目状态）。
