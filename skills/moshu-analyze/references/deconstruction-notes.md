...
> 定位注记：本文件为拆书方法论/题材知识库——「经典作品拆解/影视拉片/抽象拆书法/多线叙事/素材转化」等节属背景参考资产（B6 式定位声明，无硬消费点），保留不删。

## 管道阶段映射

| 本文件章节 | 适用管道阶段 | 用途 |
|-----------|-------------|------|
| 核心情绪控制 | Stage 2-2（黄金三章）| 判断开篇情绪是否到位 |
| 题材边界与核心梗 | Stage 2-1（概要提取）| 识别题材类型和核心梗 |
| 拆书方法论 | Stage 2-2/2-3（三章+逐章）| 拆解视角和提取方法 |
| 结构设计与节奏 | Stage 2-4（聚合分析）| 节奏地图和情绪曲线构建 |
| 卖点循环设计 | Stage 2-4（聚合分析）| 爽点循环模式识别 |
| 桥段词表 | Stage 2-4（聚合分析）| 给剧情模块打桥段标签 |
| 经典作品拆解 | Stage 2-2/2-3 | 对标拆解参考框架 |
| 拆书实操技巧 | Stage 2-3（逐章摘要）| 情绪折线图、期待分析 |
| 对标书选择 | Stage 2-1（概要提取）| 确定拆解对象 |
| 抽象拆书法 | Stage 2-4（聚合分析）| 三幕式结构归纳 |
| 影视拉片与情绪控制 | Stage 2-3 | 情绪曲线精细化分析 |
| 多线叙事与结构循环 | Stage 2-4（聚合分析）| 多线交织识别 |
| 题材实战 | Stage 2-1/2-2（概要+三章）| 题材特有爽点形态 |
| 人设塑造 | Stage 2-5（设定+关系）| 角色档案和出场设计 |
| 素材转化 | Stage 2-1~2-7（全流程）| 拆文结果转化为写作指导 |
...
| 错误 | 原因 |
|------|------|
| 拆当前作品自身 | 导入/自拆分析不是对标候选，不得复制回 对标/ |
| 只拆唯一头部爆款 | 头部作品可能依赖不可复制的作者品牌或历史流量，优先选择同平台同题材、数据稳定、非唯一头部的可复制样本 |
| 跨网站对标 | 不同网站读者群体不同，写法差异大 |

流程（单主对标架构）：扫榜收集同题材市场样本（moshu-scan 选题决策）→ 拆书 Stage 1 按题材推荐 2-3 本候选 → 用户选定 **1 本主对标**（情绪/节奏/正文权威）→ 登记 `设定/基本设定.md`（旧书题材定位.md）主对标书；无对标合法（理论 references + 八节点自排）
...
给 Stage 2-4 的 `剧情/{标题}.md` 打「桥段标签」用的常见桥段词。命中就标，逗号分隔；不在表内但本书确有的桥段照实写，不硬凑、宁缺毋滥。**可扩展种子，不是封闭枚举。**
