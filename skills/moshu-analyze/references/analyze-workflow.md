...
3. **询问用户**（用 AskUserQuestion 风格的明确二选一）：
   ...
   - 选「就到这里」→ 管道结束，`_progress.md` 状态保持 `paused_after_stage1`，告知用户「之后可随时 `/moshu-analyze` 同一本书，会自动从 Stage 2 续跑」。
...
- **检查点机制（防无限拆解）**：Stage 2 开始时（用户未预先声明"一次跑完/别问"时）先 AskUserQuestion 确认检查点间隔：

  > 「逐章摘要的检查点间隔？（每拆满 N 章停一次，报告进度+质量统计并询问是否继续）」
  > - **每 50 章停靠（推荐，适合 1M 窗口）**——约 7-10 批，停靠点给质量统计
  > - 每 30 章停靠（适合 200K 窗口 / 首次拆解保守）
  > - 每 100 章停靠（仅大窗口且对质量有信心）
  > - 一次跑完不询问（仅保留每 50 章进度播报，靠断点兜底）

  用户预先说「完整拆解/一次跑完/别问」→ 跳过询问，走"一次跑完"行为（仍每 50 章播报一次进度，不询问）。
