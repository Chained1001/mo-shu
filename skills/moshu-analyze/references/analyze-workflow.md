...
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色+**关键信息与扩写技法**+**逐章写法公式**）。逐章写法公式必须提取情绪流向、节奏配比、结构公式、核心技巧、章尾卡点与伏笔。角色过滤（龙套不提取、别名归类）。每章10-40情节点（密度150-200字/个，按字数动态调节；公式低于10时仍按硬下限10拆足关键步骤）。**并行模式：每章 spawn moshu-chapter-extractor agent**。**计数验证：摘要数 == 章节数，不等则标记失败章节**。 | 所有章节处理完成 |
...
操作：
- **优先调用标准解析脚本** `scripts/chapter_boundary.py`（`--input {原文} --outdir {拆文库/{书}/} --book {书名}`）——它一次完成章节/卷解析（阿拉伯+中文数字，含 千/两）、连续性校验、每章字数（去空白精确字符）、落盘 `章节边界.md` 与 `_progress.md` 骨架。**禁止临时手写解析脚本**（中文数字转换、混用格式、卷内重起易踩坑，2026-08 实测两轮 bug）。
...
- 脚本已生成的 `章节边界.md` 与 `_progress.md` 骨架即为切片真值；骨架内的管道进度/断点节由后续阶段按 [pipeline-ops.md](pipeline-ops.md) 更新规范维护
...
- `grep -cE '^\*\*概要\*\*' _章节摘要汇总.md` == 摘要文件数（`**概要**` 每章一行，moshu-chapter-extractor 并行输出与串行摘要模板都有；不用 `## 第N章` 头——串行摘要模板没有章节头，会误判）
