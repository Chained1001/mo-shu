...
3. **询问用户**（用 AskUserQuestion 一次问两个问题，避免二次打断）：
   ...
   - **问题 2：若继续，逐章摘要的检查点间隔？**（选项：每 50 章（推荐，1M 窗口）/ 每 30 章（200K 窗口或保守）/ 每 100 章（大窗口）/ 一次跑完不询问——间隔含义与依据见 Stage 2「批量策略」检查点机制。）选「就到这里」时本答案忽略。
...
- **检查点机制（防无限拆解）**：检查点间隔**优先在 Stage 1 停靠点询问时一并取得**（见「Stage 1 停靠点」问题 2），Stage 2 直接按该间隔执行，**不重复询问**。仅在以下情况 Stage 2 开始前补问一次：
  - 停靠点被跳过（用户一开始说"一次跑完/别问"）→ 不询问，走"一次跑完"行为（每 50 章播报进度，不询问）；
  - 续跑路径（`paused_after_stage1` 后续跑 / 分块中断恢复）且 `_progress.md` 断点未记录间隔 → 先补问间隔再开始。
  间隔含义：
