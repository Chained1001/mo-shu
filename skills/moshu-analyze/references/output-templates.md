...
本文件按 moshu-analyze 唯一管道的阶段编号组织：Stage 2-1~2-7 各阶段输出模板 + 停靠点「快速预览报告」模板 + 跨阶段通用速查。
...
Stage 2-2 停靠点交付物，输出 `拆文库/{书名}/快速预览.md`。**字段全部取自 Stage 2-1/2-2 已产出数据**（概要.md + 黄金三章深度拆解），不引入需 Stage 2-3~2-7 才能算出的字段。与终态 `拆文报告.md` 向上兼容——「基本信息」「黄金三章评分」两节同名同结构，全量拆完后 `拆文报告.md` 是 `快速预览.md` 的超集。

```markdown
# 快速预览：{书名}

> 基于黄金三章 + 全书概要的早期判断。完整拆解见后续 Stage 2-3~2-7。
> 状态：已完成 Stage 2-1/2-2，{已暂停 / 已继续全量拆解}

## 基本信息

书名 | 题材 | 总章数 | 总字数 | 目标平台（源书所在平台，拆解时识别，无则留空）

> 总字数口径 = 「章节边界」表统计的字符数（去空白），不以文件字节数或口头估算为准（UTF-8 中文约 3 字节/字）。

## 黄金三章评分

直接复用 Stage 2-2 三章深度拆解结论，5 维评分表：
...
- 开篇钩子类型与效果（来自 Stage 2-2 第1章「开篇钩子」）
- 主角立人设手法
- 世界观铺设方式
- 黄金三章节奏：三章各自功能
...
来自 Stage 2-2 各章「可借鉴要素」的汇总，3-5 条。
...
## Stage 2-1 概要（thin first-pass）

> **范围说明**：Stage 2-1 只能基于章节标题 + 卷段结构 + 抽样开头/结尾产出 thin first-pass（~200 字）。**禁止**在此阶段写出 500-1000 字 plot-aware 概要——读取范围不足以支撑那个密度。完整版在 Stage 2-6 落盘覆盖（见本文件「Stage 2-6 拆文报告 + 全书概要」节）。

输出 `概要.md`：总字数{X}万 | 总章数{N} | 题材{类型}

> 概要/卷段/章节索引的字数列均为**展示值**（精确到 0.1 万）；精确字符数以「章节边界」表（`_progress.md`）为准（Stage 2-3 情节点密度公式消费边界表的精确值，不消费这里的舍入值）。
...
> Stage 2-6 完成后会就地覆盖此文件的「首版概要」段为「全书概要（500-1000字 plot-aware）」。章节索引和卷段表保留不变。
...
**爽点分析**：类型{装逼打脸/逆袭反转/获得机缘/信息差碾压/情感满足/扮猪吃虎} | 铺垫{X}字 / 释放{Y}字 / 铺放比{Z:1} | 情绪曲线
...
P{序号} **{标题}**：类型{转折点/信息揭示/冲突/解决/铺垫/行动/对话/状态变化} | {白描一句话：谁做了什么、结果如何；原文给出起因或理由的一并写进来，不推测动机；埋伏笔的写出伏笔线索} | 涉及{全名，多人逗号分隔；纯环境铺垫无具体人物时保留"涉及"标签、值写「无」} | 地点{如明确} | 物品{如涉及} | 时间{如明确}
...
>
> **枚举单一权威**：类型/基调/主题标签的枚举值以 `scripts/check_chapter_summary.py` 为唯一权威（硬检查按它判越界）——模板此处只是使用说明，改枚举先改脚本，再同步本模板与 agent 模板。
>
...
>
> 末行硬约束：`基调：` 全角冒号不可省略（Stage 2-7 技法总结按它 grep）；`主题标签` 不加冒号；两者各取上列枚举、勿混（温馨/紧张/甜是基调、非主题）；都不贴合用「其他」。
>
...
**Stage 2-3 输出自检**（并行 agent 与 solo/direct 串行两条路径同一份要求）：概要为按时序的连贯叙述（非条目罗列、非同一连接词反复串联）｜每个情节点白描客观且写全事实/结果/原文给出的起因｜标题为 ≤15 字短标签且不与白描同句｜情节点严格按时序｜情节点数 10-40（按字数动态调节，硬下限 10）｜关键转折/关键台词/写法样本类情节点留了引用或 `原文定位`，全章不超过 8 条｜角色用全名｜类型/基调/主题标签只取规定枚举｜角色描述不跨章｜`关键信息与扩写技法` 与 `逐章写法公式` 齐全。
...
Stage 2-4 除剧情文件外，必须写一个轻量索引 `剧情/README.md`，再分别写两个权威文件：
...
> 权威范围（写作/构建流程消费）：关键信息推进、爽点循环、情绪触动点、爆发节奏。其余内容（「全书情绪节奏总览」、冲突升级路径、伏笔跨章地图、「循环单元」节等）为作者学习参考，写作/构建流程不读取。`拆文报告.md` 只引用本文件摘要。
...
兜底六步唯一权威见 [material-decomposition.md](material-decomposition.md)「散落情节兜底」；本处仅保留落盘模板骨架。

落盘 `散落情节.md`：按章节排列，标注未归类原因。

**定位声明**：`散落情节.md` 是剧情聚合的归类兜底（无法归入剧情单元的情节点），仅 moshu-import 迁移消费（合并进卷纲/大纲附录）；写作/构建环不读取。
...
> **Stage 2-5 直接按主题拆分输出多个文件**（与当前导入、长篇写作项目结构对齐）。下游不再现场拆分扁平文件。
...
输出 `拆文库/{书名}/技法总结.md`。拆解结论的应用视图（怎么学这本书），**纯作者学习材料——写书流程不读取技法总结**（写作参考走 `剧情/情绪模块.md` / `剧情/节奏.md` / 拆文报告 / `文风库/文风.md`）。**写作风格（句长/标点/对话/锚点）不在此产出**——那是独立技能 `moshu-style` 的 `文风库/文风.md`。

**字段速查**：

> confidence 档位（high/med/low 判定规则）唯一权威见 [technique-summary-sop.md](technique-summary-sop.md)「Stage 2-7-4」——本表「必标可信度」列仅列是否必标，不重复档位规则。

| 段落 | 内容 | 必标可信度 | 上限 |
|---|---|---|---|
| 生成记录 | 参考了哪些资料 + 情绪交替统计范围 + 技法可用：是/否 | 否 | <100 字 |
| 情绪交替模式 | 章内基调切换 / 跨章基调周期 / 喜剧↔重击转场 | 是（章内切换必标） | ~400 字 |
| 可借鉴技巧 | 写法技巧 Top 5 + 可借鉴套路 Top 3（从拆文报告引用；情绪/节奏意图只引用 `剧情/情绪模块.md` / `剧情/节奏.md`，不在技法总结中重定义） | 否 | ~300 字 |
| 分层学习路线 | 基础层 / 进阶层 / 适配层，各写可执行边界 | 否 | ~200 字 |
| 不可模仿 | 对标书缺陷或不适合本项目的技法 | 否 | <100 字 |

**总上限 ~1000 字**。

完整模板与生成方法见 [technique-summary-sop.md](technique-summary-sop.md)。
