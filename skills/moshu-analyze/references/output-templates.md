...
本文件按 moshu-analyze 唯一管道的阶段编号组织：Stage 0-6 各阶段输出模板 + 停靠点「快速预览报告」模板 + 跨阶段通用速查。
...
> 概要/卷段/章节索引的字数列均为**展示值**（精确到 0.1 万）；精确字符数以「章节边界」表（`_progress.md`）为准（Stage 2 情节点密度公式消费边界表的精确值，不消费这里的舍入值）。
...
>
> 末行硬约束：`基调：` 全角冒号不可省略（Stage 6 技法总结按它 grep）；`主题标签` 不加冒号；两者各取上列枚举、勿混（温馨/紧张/甜是基调、非主题）；都不贴合用「其他」。
>
