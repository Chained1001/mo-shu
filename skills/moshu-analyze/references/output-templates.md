...
## 元数据头（B73，每批拆解产物落盘时附于首个产物文件首部）

- 题材框架：{genre-catalog 路由初判框架名}（供 build 对标分流与防撞消费）
- 拆解日期：{YYYY-MM-DD}
...
**对话 DNA**（B59，作派级五要素——学结构不抄血肉）：口头禅功能位{句} | 句长习惯{短促/绵长/混用} | 语气词与标点{特征} | 回避话题{什么绝口不提} | 情绪外显{方式}
...
> **格式权威声明**：摘要格式规格权威=部署包 agent 模板 `moshu-chapter-extractor.md`（样例/白描铁律/字段规范/输出自检以彼为准，本处仅留骨架）；基调/主题标签枚举权威=`scripts/check_chapter_summary.py`（改枚举先改脚本，再同步模板与文档）。

输出 `章节/第{N}章_摘要.md`（落盘骨架）：

**概要**：{100-300字，单行自然段，按时序讲清事件/原因/结果}
**关键事件**：1.{事件} 2.{事件} 3.{事件}
...
粒度：太粗（整段打架）→丢细节；太细（举起右手）→碎片化。恰当：完整戏剧事件（"主角三招击败反派，围观者震惊"）。

P{序号} **{标题}**：类型{…} | {白描一句话} | 涉及{全名，多人逗号分隔；纯环境铺垫写「无」} | 地点{如明确} | 物品{如涉及} | 时间{如明确}
{可选引用行：≤400字原文直接引用，不加"原文引用："标签；过长或分散时改 `原文定位：{5-15字可 grep 原句片段}`}
主题标签{…} | 基调：{…}
