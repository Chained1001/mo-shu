...
并行/降级策略见 [analyze-workflow.md](analyze-workflow.md)「Stage 2 并行 Agent 策略」。
...
> **语料读取**：本阶段及阶段 4a/4c、散落情节兜底里的「全部章节摘要 / 全部情节点」统一从 `_章节摘要汇总.md`（Stage 2 收尾按章号无损拼接的全书摘要，见 [analyze-workflow.md](analyze-workflow.md)「Stage 2 收尾：合并章节摘要」）**一次性读取、在上下文中复用**，不逐文件 glob 重扫——同一份语料的 4-5 次冷读降为 1 次。汇总文件缺失或语料超上下文时，按 [analyze-workflow.md](analyze-workflow.md)「Stage 2 收尾」的分块策略执行——主线程不逐章读原始摘要，按 10-20 章/批 spawn 子代理降维聚合，不回到逐文件重扫（大书逐文件扫描会撑爆上下文）。
...
Stage 3 聚合后必须生成两个权威产物，供 Stage 5 报告摘要和 `moshu-write` 直接读取：
...
Stage 2 使用 moshu-chapter-extractor agent 并行处理（每章一个 agent，每批 5-8 个），不分块。
...
输出长度限制：Stage 2 agent 模式按密度公式输出，Stage 3-5 单阶段 ≤8000 中文字符。每批完成后更新 _progress.md。
