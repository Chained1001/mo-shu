...
> **语料读取**：本阶段及阶段 4a/4c、散落情节兜底里的「全部章节摘要 / 全部情节点」统一从 `_章节摘要汇总.md`（Stage 2 收尾按章号无损拼接的全书摘要，见 [analyze-workflow.md](analyze-workflow.md)「Stage 2 收尾：合并章节摘要」）**一次性读取、在上下文中复用**，不逐文件 glob 重扫——同一份语料的 4-5 次冷读降为 1 次。汇总文件缺失或语料超上下文时，回退原「扫描全部章节摘要」逐文件方式 / 分块策略，行为不变。
