...
> **定位注记**：章节分隔识别已由 `scripts/chapter_boundary.py` 脚本化承担（analyze-workflow 禁手写解析）；本节为方法论背景说明。
...
并行/降级策略见 [analyze-workflow.md](analyze-workflow.md)「Stage 2-3 并行 Agent 策略」。

Agent 输出格式严格对齐本阶段 A/B/C 三部分的合并输出，详见 output-templates.md 的 Stage 2-3 模板。

#### A. 章节摘要

- 章节概要（100-300 字，按时序连贯讲清事件、原因、结果；写法与自检见 output-templates.md「Stage 2-3 章节摘要+情节点」）
- 关键事件（3-5 个，按时间顺序）
- 关键信息与扩写技法
- 逐章写法公式（情绪流向/节奏配比/结构公式/核心技巧/章尾卡点与伏笔）
- 出场人物（情节点行内带主题标签与基调，无章节级字段）
...
> **语料读取**：本阶段及阶段 4a/4c、散落情节兜底里的「全部章节摘要 / 全部情节点」统一从 `_章节摘要汇总.md`（Stage 2-3 收尾按章号无损拼接的全书摘要，见 [analyze-workflow.md](analyze-workflow.md)「Stage 2-3 收尾：合并章节摘要」）**一次性读取、在上下文中复用**，不逐文件 glob 重扫——同一份语料的 4-5 次冷读降为 1 次。汇总文件缺失或语料超上下文时，按 [analyze-workflow.md](analyze-workflow.md)「Stage 2-3 收尾」的分块策略执行——主线程不逐章读原始摘要，按 10-20 章/批 spawn 子代理降维聚合，不回到逐文件重扫（大书逐文件扫描会撑爆上下文）。
...
识别方法：扫描概要.md（Stage 2-1 thin first-pass 足够，无需等 Stage 2-6 全书概要）+ 前3章摘要 + 后3章摘要，判断核心驱动模式。
...
Stage 2-4 聚合后必须生成两个权威产物，供 Stage 2-6 报告摘要和 `moshu-write` 直接读取：
...
#### D. 小说概要生成（在 Stage 2-6 落盘，覆盖 Stage 2-1 thin first-pass）

整体故事框架识别 + 500-1000字高密度全书概要（涵盖主要剧情线阶段性发展、核心人物作用、关键转折点、因果关系）。

> **落盘时机**：聚合阶段（Material 阶段 3 ≈ pipeline Stage 2-4）产出概要文本，但**写入 `概要.md` 的动作放在 pipeline Stage 2-6 汇总报告**——届时 Stage 2-4 / 4 全部完成、信息最完整。Stage 2-6 覆盖 Stage 2-1 的 thin first-pass 段，章节索引和卷段表保留。

### 阶段 4：世界观与设定提取

Stage 2-3 的角色轻量提及数据在此阶段升级为完整档案。

#### 角色两阶段模型

**阶段 A：轻量提及（Stage 2-3 输出）**
- 每个 agent 返回出场人物表（姓名/重要性/别名/本章表现）
- 不做跨章分析，只记录本章可见信息

**阶段 B：完整档案（Stage 2-5 构建）**
- 合并所有章节的角色提及数据
- 跨章节别名解析
- 构建完整角色档案
...
> 模板与示例见 [output-templates.md](output-templates.md) 「Stage 2-5 设定+关系」节。下游导入只原样同步当前主题文件，不再现场拆分扁平文件。
...
**隐含关系推断**：当角色间没有直接互动但有间接证据时，按以下档位标注置信度（定性档位，非机检门限——不存在脚本计算这组比值，数字是 AI 自检口径）：
- 通过第三方的评价/描述推断（置信度标 强推断）
- 通过共同出现的频率和场合推断（置信度标 中推断）
- 推断关系标记 `[推断]`，与直接证据关系区分
...
目录结构见 [analyze-workflow.md](analyze-workflow.md)「输出目录结构」。
...
| 关键信息覆盖 | >= 每章1条 | 含 `关键信息与扩写技法` 的章节数 / 摘要章节数 | 缺失章节回到 Stage 2-3 补提取 |
...
> **口径声明（审计-V3 AM3）**：本表比值为 **AI 手算的自检口径，非机检门限**——没有脚本计算这组除法，数字只供主线程自查与复核定位用；置信度数值是"像测量值的估计值"，改动时勿把它当确定性判据（三层分工：脚本做确定性、AI 做语义）。
...
Stage 2-3 使用 moshu-chapter-extractor agent 并行处理（每章一个 agent，每批 5-8 个），不分块。
Stage 2-4~2-6 的分块/批次策略如下（2-1 概要+边界表、2-2 黄金三章原文量小，直接整体处理，不适用分块）。
...
输出长度限制：Stage 2-3 agent 模式按密度公式输出，Stage 2-4~2-6 单阶段 ≤8000 中文字符。每批完成后更新 _progress.md。

### 处理批次（一次读多少）

Stage 2-4~2-6 的语料是 `章节/*_摘要.md`。**实测体积：中文章节摘要约 15KB/章 ≈ 5-7K tokens/章**（据大奉打更人 57 章样本，869,663 字节 / 57 ≈ 15,257 字节）。按这个体积，「把全书摘要一次读进主线程」在 >500 章时必然超上下文——所以大书不是"把块调大"，而是**换成子代理并行 + 降维聚合**。

**A. 子代理并行模式（>500 章默认，Stage 2-4~2-6 通用）**
...
分块处理后，相邻块的边界剧情可能被机械切分割裂。Stage 2-4 聚合时执行跨块合并检查：
...
跨会话恢复操作唯一权威见 [pipeline-ops.md](pipeline-ops.md)「恢复机制操作步骤」（schema 校验/paused 特判/概要核对三保护不在此重复）。
