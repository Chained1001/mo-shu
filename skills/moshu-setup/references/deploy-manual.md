# deploy-manual.md：部署兜底指引（Stage 2-1~2-7 与 Stage 3 逐项验证）

> SKILL.md 正常路径走 `deploy.py` 一键部署；本文件是脚本不可用/冲突时的兜底指引与处理规则。

## Stage 2-1：部署清单（机械可检查）
...
| `skills/moshu-setup/references/templates/agents/*.md` | `.claude/agents/*.md` | moshu-setup managed | replace | agent files match template (source-target count identical，不写死数量——与 verify 动态化同口径) |
...
## Stage 2-6：合并 Hooks 注册到 settings.local.json
...
- 如果 `.story-deployed` 已存在但 `agents_version` 缺失、非整数或小于 `47`，按本次流程更新 hooks/agents/rules/reference bundle（具体变更见 `UPGRADING.md`）；大于 `47` 时已在 Stage 1 停止，不得降级覆盖
...
3. 验证 agents：`.claude/agents/` 下的 agent 定义文件与模板一致（源目标计数相同，不写死数量）
...
7. 验证部署标记：`.story-deployed` 存在且包含时间戳、`agents_version: 47`、`setup_skill_version: 1.5.1`、`target_cli`、`resolver_strategy`、`references_dir`
...
替换时去掉花括号。**只问模板实际出现的占位符**（当前模板只有 `{项目名}` 与 `{书名}`；`{目标平台}` / `{作者名}` 不在模板中——目标平台由开书（`moshu-outline` Stage 1-2）与导入（`moshu-import` Stage 1）各自采集并写入 `设定/基本设定.md`（B102 前旧名题材定位.md）权威字段，不在部署阶段重复提问）。如果用户未指定项目名，用当前目录名；书名未指定时与项目名相同。未指定的占位符保留原样不替换。
...
- `.story-deployed` 存在且 `agents_version: 47` → 提示已部署，AskUserQuestion 确认是否重新部署；提示里写明重新部署只用当前本地 skill 包刷新项目文件，skill 本身的更新走 `git pull` 或 marketplace
- `.story-deployed` 存在但 `agents_version` 缺失、非整数或小于 `47` → 提示需要更新，重新执行 Stage 2 覆盖 agents/hooks/rules/reference bundle，CLAUDE.md / settings.local.json 走合并策略
- `.story-deployed` 存在且 `agents_version` 大于 `47` → 当前 skill 版本过旧，停止并提示先更新 mo-shu；不覆盖项目中的更新部署
