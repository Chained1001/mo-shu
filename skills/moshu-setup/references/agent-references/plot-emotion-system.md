...
> **消费点（B103 刷）**：4.3 情绪引擎设计+5.2 打磨（情绪维弹药）
---
...
### 情绪低压电路（选配指引；与装逼电流并列的低压侧）

高压电路（缺口→延迟→释放→余波）之外的低压侧，三种电流：

| 低压电流 | 怎么用 | 使用场景 |
|------|------|----------|
| 幽默 | 对话/情境错位制造笑点 | 两场高压之间给读者换气 |
| 温情 | 关系回暖/并肩时刻/日常羁绊 | 高压后安抚，铺垫感情线 |
| 悲后回温 | 悲情节拍后给一个回暖落点 | 悲后不冷场，读者情绪有出口 |

**调剂密度问句**：写低压段前问"上次读者笑/暖/缓是什么时候？"——连续高压过久先补低压，连续低压过久回高压。

**严肃题材低配豁免**：悲凉/苦情/硬核流题材可不设幽默电流（温情/回温仍可低配保留）——低压电路是选配指引，非强制。
...
- **密度缺省参考（C22①，外部经验值无书证——有对标/采风标尺时以其为准）：小爽点≥每 2000 字 1 个；爽点间隔≤5000 字；大爽点需数章铺垫**。——本组数字作**可覆盖的缺省兜底**（无数据时起手参照），不作设计目标（区别于已弃用的 60/30/10 配比）。
...
> **铺垫质量三问（C22②）**：①立靶了吗（反派/路人的轻视刁难）？②主角压到位了吗（表面示弱/处境受限）？③反差够吗（是否用前文伏笔翻盘）？
...
**故事构型 + 素材 = 具体故事**。用法两种：①**重构**（保留素材，彻底改构型，长篇必备）；②**微调**（用经典情节，按人设深挖微调）。人设与经典情节冲突时必须重构。
