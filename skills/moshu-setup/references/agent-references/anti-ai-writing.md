...
## 文字打磨纪律（B95，自 deslop 收编）

- **改味三条原则**：改味优先别当改错（功能性写法标 `[需复核]` 保留）/ 改最少效果最大（只改损害读感且无叙事功能的问题）/ 过度去 AI 味保护（不得整段删；删除前确认不丢伏笔钩子角色特征情节推进情绪承接因果锚点）。
- **保留清单红线**：`preserve` 只有**作者**与 `[需复核]` 转正两个来源（reviewer/改稿执行不得自填，防「自我保留」架空整改）——执行细节见 write 侧 workflow-revision 文字打磨路「外科手术纪律」。
- **豁免标记注记（B95 自 deslop 迁移）**：文件首 6 行带 `<!-- 去味:跳过 -->`（全/半角冒号皆认）时脚本跳过扫描——**显式调用去味不得把跳过当成零命中**，应告知用户标记在场并询问是否删除标记后重扫；豁免只作用于 check-ai-patterns，degeneration/标点/细纲照搬三网照常。
...
不要用双引号给普通名词、常见动作或作者临时概括的概念做“引号强调”。这类写法会把没有特殊含义的词硬包装成术语，连续出现时尤其像模型在替读者划重点。`quote-emphasis-tic` 规则只负责提示，最终按语境判断。
...
> **爆款语料校准**（七猫长篇 现言/都市/古言/玄幻/历史 125 本×前 8 章旁白统计）：逗号之间平均 8.8-9.6 字；整句平均 22-24 字；逗号长句占叙述句 74-80%；≤5 字的短片段约占两成，多是孤立的时间词、转折、动作重拍。**句子内部不碎**。
...
### 3. 格式一致性
- 对话格式统一：按项目/平台约定保持同一引号风格；古言/日式可用「」
...
> **两句自检（AI 味照妖镜，写完每段用）**：
> - **句子可追问检查**：写完每句问"然后呢？具体是什么？"——经不起追问的句子（"夜色很美""他心情复杂"）重写。空洞句子经不起追问。
> - **句级信息增量检查**：删掉这一句，读者损失什么新信息？答案是"没损失"就删。孤立重拍短句可只承担节奏，但成片碎句中每句至少要传递一个上下文中不可预测的信息。
...
| 元叙事旁白 | 「故事发展到这一步」「读者也许会想」「接下来发生的事」「到这里算是」-> 删。你是亲历者不是编剧，只写角色此刻正在经历的事 |
| 升华式收尾 | 结尾对仗拔高、金句点题 -> 用一个动作或一句留白收住，把"意思"压进画面里 |
| 抽象命运/开端收束 | 「命运终于露出獠牙」「早已布好的棋局」「这一刻终于明白」「属于他的反击才刚刚开始」-> 改成角色当下可见的文件、动作、对话或物理后果；确定性 AI 句式检测报 `abstract-summary-tic` 时优先处理 |
...
自检：每句问一遍——这是"角色在经历"，还是"作者在讲解/安排"？凡作者跳出来讲，删，或改成角色视角内的呈现。根治办法是锁定深度限知视角，镜头钉死在角色身体里，作者就没位置跳出来了。
...
| 「扯了下/停了一下/拍了两下/松了半圈」式微动作高密度复现（确定性 AI 句式检测报 micro-action-tic） | 合并动作，换具体细节；不是每个动作都要接一个反应尾巴 |
...
确定性 AI 句式检测的 `action-list-tic` 只提示监控动作清单，不是 blocking。功能性打斗/追逐/仪式步骤若动作链本身承担信息，可保留或标 `[需复核]`。番茄高分样本中该类命中为 0，因此适合作为“需通读”的风格提示，而不是硬性失败项。

#### 工具提示处理

确定性 AI 句式检测是本地写作 lint；blocking 只限确定性句式/标点问题，advisory 不作完成门槛。用户贴其他工具报告时，只把能落到正文的句式、段落、词汇问题转成具体修改点，不写“0% AI / 100% 真人”或“固定公式”，也不围绕分数反复微调。
...
"不知道为什么，但……""什么都没说，却……""明明没有理由，却……"——用认识论悖论代替情感承载，是 AI 哲学腔。

- 处理：情感靠行动/对话承载；角色自己不知道 → 写他不知道时的行为（发呆、反复看同一物、答非所问）。
- 示例：「他不知道为什么，却停下脚步」→「他停下脚步，鞋尖在门槛前碾了碾，半天没迈进去」

### 模式 12：物化结尾

场景/章节/段落结尾**禁止用比喻将人变成物体或自然现象**制造余韵（"他像一片落叶飘远了""她化作了一阵风"）——物化人物是 AI 制造"文学感"的惯用手法。

- 处理：用具体行动、一句话或物件细节收束；确需比喻时保留人物主体。
- 示例：「他像一片落叶，被风卷走」→「他转身走进雨里，风掀起外套下摆，又落下」
...
## 剪去 90% 与套路复读警示（B78，大师写作课 Ch.5/14 苏格拉底化）

**可信度=过程细节完整度，非形容词浓度**——读者信一段打斗，是因为招式间的**过程细节**（重心变化/武器手感/呼吸节奏）完整，不是堆形容词；检测到「形容词密、过程疏」段落 → 剪去 90% 的修饰词，把省下的字数补过程步骤。

**重复排练≠重新创造**——同一情绪第二次出现时若只有外部形式相似而无**内在情绪序列**推进（套路复读）；复用场景模板时必须重走情绪导出（见 emotional-methods 规定情境法），导不出新序列=复读，重写。
