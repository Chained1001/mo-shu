...
| ★★★☆ | 翻案腔变体："与其说A不如说B" / "看似A实则B" / "表面A实际B" / "你以为A其实B" / "A不重要，重要的是B" / "回头才发现" / "答案恰恰相反" | "你以为他赢了，其实他输得更彻底" | 判断从正面直接下：先给判断，再给依据；只有文章真的用材料走过误解→修正过程才保留一次自我修正 |
...
### 模型路标词（叙述中替换，台词可保留）
说白了、说穿了、先说结论、说到底、更微妙的是、还有一层、只说对了一半、值得注意的是、需要指出的是、从某种意义上说（角色口语/真实语境可保留；叙述里当洞察路标抬价时删掉，把判断直接接进上下文）
...
| 动词名词化："完成了对…的优化 / 实现了…的提升 / 起到了…作用" | "完成了对流程的优化" | 公文腔；写回直接动词（把流程改顺了） |
| 提示性冒号："一句话总结： / 核心是： / 重点是：" | "一句话总结：他赢了。" | 冒号只保留引出人物直接原话；提示性冒号删掉或改逗号 |
