...
>
> **证据摘要口径（B71）**：各卡尾部样本数据（可用本数/抽样/占比）系上游项目快照，语料不可复现——按【文档宣称】使用，作参考量级不作精确依据；置信度为上游标定，可由 settings 评审复核（B69）。
...
1. 先读 `设定/基本设定.md`（旧书题材定位.md），确认主题材、目标平台、性向频道、主对标书和核心梗。
...
4. 卡片用于**每章 JIT 组装**的 `genre_prose_card`（B102 ⑱㉔：Stage 1 不再生成提示卡文件，卡不落盘）；中置信卡需结合本书对标/细纲对照；低置信卡只能兜底，必须标注低置信并优先补同题材对标。
5. 每章传给 moshu-narrative-writer 的 `genre_prose_card` 只保留本章相关条目，推荐 120-300 字；包含：题材限制、核心逻辑、读者期待、核心爽点/情绪、正文落点、前中后期打法、场景颗粒、禁止漂移、本章取舍、卡片置信度。
...
本地长篇样本不支持固定 50-60 字行宽、固定 50%-60% 对话占比、全局替换“地/得/很/像/顿号”、随机倒装、三番八情或三翻四震等机械模板。题材卡只给“该题材更常用的场面和情绪落点”，具体段落仍按细纲、文风和当前场景处理。
