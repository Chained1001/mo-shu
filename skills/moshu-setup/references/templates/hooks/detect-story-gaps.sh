...
for BOOK_DIR in "${BOOK_DIRS[@]}"; do
  ...
  if [ -f "$BOOK_DIR/追踪/伏笔.md" ]; then
    ...
    if [ -n "$ABNORMAL_FORESHADOW" ]; then
      BOOK_OUTPUT+="[WARN] ${BOOK_NAME}：伏笔.md 中检测到过期或异常的伏笔条目，建议跑 /moshu-review lean 或做一次伏笔审计。${NL}"
    fi
  fi
...
if [ -d "$ROOT/拆文库" ]; then
  ...
  while IFS= read -r progress_file; do
    ...
    GLOBAL_PROGRESS_OUTPUT+="[WARN] 拆文未完成：${progress_file#$ROOT/}，运行 /moshu-analyze 继续。${NL}"
