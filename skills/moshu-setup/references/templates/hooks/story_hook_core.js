...
function trackingCheckpointIssue(book, requireState = false, expectedLastCommitted = null) {
  ...
  if (!fs.existsSync(state)) {
    return requireState
      ? `追踪/_tracking-state.json 缺失；已有正文项目走 /moshu-import 的「旧追踪项目迁移」重建追踪（不必重跑全书拆解），新书先用 tracking_commit.py init 初始化`
      : null
  }
  let document
  try {
  ...
  if (!document || typeof document !== "object" || Array.isArray(document) || document.schema_version !== 6) {
    return `追踪/_tracking-state.json 不是当前 schema_version=6；停止写正文并重新 /moshu-import，不保留旧结构兼容路径`
  }
  ...
  if (expectedLastCommitted !== null) {
    if (!Number.isInteger(document.last_committed_chapter)) {
      return `追踪/_tracking-state.json 缺少整数 last_committed_chapter；停止写正文并重新 /moshu-import`
    }
...
function proseBlockReason(root, absolute) {
  ...
  // 让核心守卫 fail open。
  // moshu-import 在复制既有正文、尚未执行 tracking init 的窗口可以写；一旦 state 存在，
  ...
  if (!exists) {
    ...
    if (!found) {
      return `⛔ 写正文被拦截：第 ${chapter} 章缺少细纲（${safeRelative(root, outlineDir)}/细纲_第${chapter}章.md）。先按 moshu-write 单章流程补建细纲再写正文。`
    }
  }
