  ...
  case "$AGENTS_VERSION" in
    ...
    *)
      if [ "$AGENTS_VERSION" -lt 27 ]; then
        OUTPUT+="[WARN] moshu-setup agents_version=$AGENTS_VERSION 低于 v27。重新运行 /moshu-setup 刷新 hooks、agents 和 references（部署后需新开会话）。${NL}${NL}"
        HAS_CONTENT=true
      elif [ "$AGENTS_VERSION" -gt 27 ]; then
        OUTPUT+="[WARN] moshu-setup agents_version=$AGENTS_VERSION 高于本 hook 支持的 v27。不要降级覆盖；请先更新 mo-shu。${NL}${NL}"
        HAS_CONTENT=true
      fi
      ;;
  esac
