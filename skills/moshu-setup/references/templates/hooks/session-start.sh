...
if [ ! -f "$HOOK_DIR/lib/common.sh" ] || [ ! -f "$HOOK_DIR/lib/sentinel.sh" ]; then
  printf '%s\n' "[WARN] moshu hook 函数库缺失。重新运行 /moshu-setup 恢复 .claude/hooks/lib/。"
  exit 0
fi
...
# moshu-setup 部署后的一次性重启确认。custom agents 只在会话启动时被注册成
...
if sentinel_exists "$ROOT/.story-deployed"; then
  ...
  if [ -n "$MISSING_HOOKS" ]; then
    ...
    OUTPUT+="  修复：重新运行 /moshu-setup 恢复缺失的 hook。${NL}${NL}"
    HAS_CONTENT=true
  fi
  ...
  case "$AGENTS_VERSION" in
    ''|*[!0-9]*)
      OUTPUT+="[WARN] .story-deployed 缺少数字 agents_version。重新运行 /moshu-setup。${NL}${NL}"
      HAS_CONTENT=true
      ;;
    *)
      if [ "$AGENTS_VERSION" -lt 38 ]; then
        OUTPUT+="[WARN] moshu-setup agents_version=$AGENTS_VERSION 低于 v38。重新运行 /moshu-setup 刷新 hooks、agents 和 references（部署后需新开会话）。${NL}${NL}"
        HAS_CONTENT=true
      elif [ "$AGENTS_VERSION" -gt 38 ]; then
        OUTPUT+="[WARN] moshu-setup agents_version=$AGENTS_VERSION 高于本 hook 支持的 v38。不要降级覆盖；请先更新 mo-shu。${NL}${NL}"
        HAS_CONTENT=true
      fi
      ;;
  esac
  ...
  for field in setup_skill_version target_cli resolver_strategy references_dir; do
    if [ -z "$(read_sentinel_field "$field" "$ROOT/.story-deployed")" ]; then
      OUTPUT+="[WARN] .story-deployed 缺少 $field 字段。重新运行 /moshu-setup 刷新部署元信息。${NL}${NL}"
      HAS_CONTENT=true
    fi
  done
  ...
  if [ -n "$REFERENCES_DIR" ]; then
    ...
    if [ -n "$MISSING_REFS" ]; then
      OUTPUT+="[WARN] moshu-setup 参考资料包缺失或为空：${MISSING_REFS}。重新运行 /moshu-setup；若重跑后仍报这条，是 skill 包本身没装全，按你的安装方式重装 mo-shu（git pull 或 marketplace 面板）再部署。${NL}${NL}"
      HAS_CONTENT=true
    fi
  fi
else
  OUTPUT+="[WARN] 写作环境未部署。运行 /moshu-setup 初始化。${NL}${NL}"
  HAS_CONTENT=true
fi
...
if [ -d "$ROOT/拆文库" ]; then
  ...
  if [ "$PROGRESS_COUNT" -gt 0 ]; then
    OUTPUT+="[INFO] 拆文库/ 中有 $PROGRESS_COUNT 个未完成拆文。运行 /moshu-analyze。${NL}"
    HAS_CONTENT=true
  fi
fi
...
story_update_check() {
  ...
  if [ "$((now - last))" -ge 86400 ]; then
    checked=1
    latest=$(curl -fsS --max-time 5 "https://api.github.com/repos/Chained1001/mo-shu/releases/latest" 2>/dev/null \
  ...
  if [ "$latest" != "$cur" ] && [ "$(printf '%s\n%s\n' "$cur" "$latest" | sort -t. -k1,1n -k2,2n -k3,3n | tail -1)" = "$latest" ]; then
    OUTPUT+="[INFO] 网文工具箱有新版本 v${latest}（当前 v${cur}）。更新：npx skills add Chained1001/mo-shu -y -g 后重跑 /moshu-setup；或对 /moshu 说“检查更新”。关掉提醒：export STORY_NO_UPDATE_CHECK=1${NL}"
    HAS_CONTENT=true
  fi
}
