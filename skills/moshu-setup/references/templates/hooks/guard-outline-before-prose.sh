...
case "$BASE" in
  *)
    ...
    # moshu-import 迁移：已有 拆文库/{书名}/ 分析源时放行（细纲由章节摘要反推、晚于正文迁移）。
    ...
    if [ -z "$EXISTS" ]; then
      ...
      if [ -z "$FOUND" ]; then
        ...
        printf '%s\n' "   按 moshu-write 单章流程先补建细纲，再写正文（不允许跳过细纲直接写作）。" >&2
        printf '%s\n' "   如确需先起草，请先补建对应细纲文件。" >&2
        exit 2
      fi
    fi
    ...
    # 主产物门（只对首建新章判，与细纲门同序）：有对标项目时，情绪模块/节奏
    # 任一缺失即拦（与文档写前准备 fail-fast 一致；情绪/节奏轴不豁免自定义文风）。
    # 已存在的正文放行——deslop 去AI味 / workflow-revision 回炉改写不经过写前准备，
    # 不需要情绪模块/节奏（只需 anti-ai 规则），误拦会卡住合法改稿。
