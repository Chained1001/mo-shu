...
## 采风专用模式（B107——moshu-outline spawn 传入 type=caifeng|survey 时启用）

> spawn 只需传：type + {对象：主参考《X》+辅参考 或 题材+基调} + project_dir + cdp_port——检查单/schema/源优先级已内嵌本节，prompt 零重复。

**type=caifeng（有参考，逐部针对性）**四域检查单：
- 剧情：{书}故事脉络/关键转折/常用桥段/节奏快慢
- 设定：{书}世界观设定/力量体系/规则限制/世界样貌
- 人设：{书}角色类型/性格特点/关系网
- 写作技法：这类书开篇手法/悬念铺法/爽点安排/节奏控制（设计层用法）
- 无对标补查：真实人物原型/时代背景

**type=survey（无参考，题材级）**三域+谱系：
- 剧情：{题材}热门故事线/主流桥段/节奏惯例
- 设定：{题材}标配设定/力量体系惯例/常见规则
- 人设：{题材}主角类型/配角配置/关系网
- 加：代表作谱系（含 URL）+竞争格局（头部/拥挤区/空位）

**产物 schema**：有参考=参考档案九段（①故事脉络②关键剧情节点③主线支线④主要设定⑤主要人物表[功能位]⑥底色配方与模式占比⑦缺点清单⑧专名清单[防抄机检锚]⑨来源与截至日期）；无参考=综述八节（题材概况/主流模式/桥段库/设定标配/主角类型谱/读者偏好与雷区/代表作谱系[每条带 URL]/竞争格局与差异化空位）——**每节头部标来源数**（单源标「单一来源」）。

**源优先级**：发现层=搜狗 web→搜狗微信→360 按序 fallback；内容层=豆瓣/萌娘百科/晋江/搜狐号；禁用 google/wikipedia/bing/起点/知乎/番茄。CAPTCHA/验证码壳=立即切下一层。

**红线**：全书正文不取不复述（只取元数据层）；临时文件进 `设定/.caifeng-tmp/`；产物落 `设定/参考/` 与 `设定/采风/`（CF 票据）。
