...
| 场景 | 什么时候触发 | 典型查询 | 搜索要点 |
|------|------------|---------|---------|
| 素材收集·剧情域（L1） | 大纲草稿 4.2 前（Stage 2 三域深采） | 主参考故事脉络/关键节点/桥段手法 | 元数据层全覆盖（正文红线）；落参考档案「脉络/节点/支线段」 |
| 素材收集·设定域（L1） | 大纲草稿 4.2 前（Stage 2 三域深采） | 主参考世界观/力量体系/规则质感 | 时代素材（如"唐朝"）落 `设定/资料/{专题}.md` 素材层 |
| 素材收集·人设域（L1） | 大纲草稿 4.2 前（Stage 2 三域深采） | 主参考人物表/动机缺陷反差/关系网 | 落参考档案「人物表段」；学功能不学背景 |
| 定向素材收集（L1 按需） | 草稿 4.3 世界观完善 | `设定/资料/{专题}.md` 五维采集 | 条目式（每维 5-15 条）+来源红线+转译注记栏 |
| L2 工艺学采风 | 卷纲单元构建按需 | 单元结构/节拍/伏笔/支线/配角/开场钩 | 见 caifeng-volume.md（桥段表+情绪节拍必填） |
...
**产物落盘（B103 三路径）**：① `{project_dir}/设定/参考/{书}.md`（素材收集深采档案——schema 九段见 outline 侧 caifeng-outline.md「参考作品档案」节，本模板不内嵌）② `{project_dir}/设定/采风/采风-CF{NNN}-{类型}-{主题}.md`（CF 票据——schema 九块见 caifeng-outline「CF 票据制」节）③ `{project_dir}/设定/资料/{专题}.md`（时代素材五维——见 caifeng-outline「资料层五维」节）。产物结构以 caifeng-outline.md 为权威（分层引用不内嵌——防漂移）。CF 编号递增：`ls 设定/采风/采风-CF* 设定/采风-CF* 2>/dev/null` 取 max+1。

**分层引用声明（B103）**：产物 schema（九段档案/CF 票据九块/资料层五维）以 caifeng-outline.md 为权威——spawn 时主会话把对应 schema 节作为 context 传入，本模板不内嵌副本（防两处漂移）。

**输出 JSON 扩展（采风任务）**：返回 `research_file` 指向采风产物路径，`type` 取 `caifeng-story | caifeng-setting | caifeng-character | caifeng-material`——主线据 `type` 路由消费（素材收集三域/L2 工艺学）。
