...
- **正文形状由你编排（形状层）**：细纲是「要发生什么」的契约、不是正文的形状。落地位置、顺序、拆成几处、怎么断段由你定——可打散重排、把相邻几项缝进同一个连续动作；**不要一项一段按序平推**，也不要把细纲措辞原样搬进叙述（技法见 `moshu-setup/references/agent-references/writing-craft.md`「从细纲到正文」）。**例外**：细纲「复沓锚句」列出的原话要逐字落进它标注的那个情节点，不得改写或挪位置。
...
| 参考文件 | 何时读取 |
|---|---|
| `moshu-setup/references/agent-references/writing-craft.md` | 正文写作（**从细纲到正文·不照抄形状**、三维度揉进、身体细节、物件三次出现、小节密度）时 |
| `moshu-setup/references/agent-references/format-and-structure.md` | 段落节奏、正文格式规范时 |
| `moshu-setup/references/agent-references/genre-catalog.md` | 题材技法（克制/爽感路线）时 |
| `moshu-setup/references/agent-references/emotional-arc-design.md` | 情绪弧线执行、题材情绪策略时 |
| `moshu-setup/references/agent-references/genre-prose-cards.md` | 按番茄题材分类校准正文提示卡时先读索引，再只读取 `moshu-setup/references/agent-references/genre-prose-cards/{题材}.md` 单卡；卡片只内部校准题材味，正文里不出现卡片文字或合规自评 |
...
> 详细技法参考 `moshu-setup/references/agent-references/writing-craft.md`「场景写法」

**叙述姿态（默认·深度限知）**：全程锁死主视角角色的此刻感知，只写她此刻看到/听到/闻到/身体感到/脑中闪过的；镜头不拉远、不俯瞰、不切他人内心；读者与她同步获知，不提前剧透、不补全背景；念头用"闪念+身体"呈现，不写完整理性独白；场景被她的情绪染色，不写中立摄像机式描述。这条是去说教/上帝感的根（详见 `moshu-setup/references/agent-references/writing-craft.md`「视角姿态：深度限知」、`moshu-setup/references/agent-references/anti-ai-writing.md` 模式 8）。

1. **进入场景**：主角此刻在哪、在做什么（1-2 句切入）
2. **展开子事件**：每个子事件将发生、感知、反应三维度揉进同一段连续正文（详写的子事件合计 ≥100-150 字；过场/连接类 1-2 句带过，不要每个子事件平均用力，见 `moshu-setup/references/agent-references/writing-craft.md`「疏密分配」）
   ...
   - 子事件之间用身体动作连接（~20 字）
   - **画面分段**：三维度揉进不等于一段到底。按新动作/新物件/新信息/新对话断段，不按维度断段；读起来卡或多个完整动作挤在一段里就拆，连续碎段像提纲就把同一镜头的相邻句合并。拆的是段落，句子内部仍是逗号长句（见 `moshu-setup/references/agent-references/anti-ai-writing.md` 规则 3）
3. **收尾**：钩子或情绪定格（1-2 句）

关键辅助技法（均见 `moshu-setup/references/agent-references/writing-craft.md`）：
- 身体细节替代情绪词
- 贯穿道具三次出现规则：每个物件出现 3 次，意义逐次翻转
- 一动一静节奏：动作段后接静止感知段
- 疏密分配 + 长短句交错：爽点 beat 写密、过场 beat 写疏；高潮压短句、沉淀放长句，忌通篇同长度（见 `moshu-setup/references/agent-references/writing-craft.md`「疏密分配」、`moshu-setup/references/agent-references/format-and-structure.md`「段落节奏」）
- 小节密度诊断：5 项清单逐条检查

### 情绪弧线执行

> 题材情绪策略参考 `moshu-setup/references/agent-references/emotional-arc-design.md`

- 情弦理论：锁定目标读者的核心情感弦，每节至少拨一次（`moshu-setup/references/agent-references/emotional-arc-design.md` 情绪弧线）
...
> 完整开头设计见 `moshu-setup/references/agent-references/opening-design.md`

- 前 100 字事件密度 >= 3（见 `moshu-setup/references/agent-references/writing-craft.md`）
- 黄金三章法则（长篇）
...
> 完整方法见 `moshu-setup/references/agent-references/anti-ai-writing.md`
> 禁用词表见 `moshu-setup/references/agent-references/banned-words.md`

- **Gate A 禁用词替换**：命运齿轮/如潮水般/仿佛春风/心猛地一沉/眼眶泛红等全部替换（查 `moshu-setup/references/agent-references/banned-words.md`）
...
系统性去AI三遍法（`moshu-setup/references/agent-references/anti-ai-writing.md`）：
...
- 验证粒度：长篇以章为准，细纲 `字数目标` 为唯一权威，低于 90% 即未达标，节奏类型只影响情节点疏密、不另设静态最低字数。
...
### 流程规则承接（B75a——两级装配的模板侧：以下为已部署 chapter-core 稳定规则的摘要引用，完整规则见已部署的 chapter-core.md，本节不是替代）

1. **三遍法结构**：第一遍快写（创作帽——一气呵成，不回头润色）；第二遍读者帽（四查：可信/有趣/角色辨识/LOCK 情感）；第三遍技艺帽（五磨：章首钩子/意象/对话/节奏/删冗）。每遍只列判据，具体操作口径以 chapter-core 对应节为准。
2. **细纲优先边界（内容层）**：细纲是执行权威——情节点顺序与功能标签、密/疏预算分布、复沓锚句（须一字不差进正文）、「本章涉及设定」的装配清单，均按细纲执行，不得自行增删主情节。
3. **禁止提前释放纪律**：细纲「本章禁止提前释放」条目与带 🔒 剧透锁标记的伏笔/信息差，正文不得揭示或变相暗示（含旁观镜头、系统面板、他人行动的间接泄露）。
4. **格式硬约束**：字数目标落在 [目标, ×1.1]；情节点预算 Σ 同契约；章尾钩子承诺必须在章尾兑现落点。
5. **场景路由映射（B56）**：高压章→短句快切、少环境描写；低压章→长句舒缓、多感官细节；关系章→对话优先、动作辅助；信息章→条理清晰、不堆砌。
6. **一进一出判据（B68）**：本章解决一个小问题、欠下一个新问题——不许一章清账，也不许章章欠账。
7. **语声对照（B59）**：出场角色的语声锚与称谓行逐句对照——「这句台词换个人说还成立吗？成立=无语声个性」；称谓与登记一致。

### 写完后对话自检（涉及对话的章节必做）

正文含对话时，写完每章/每节后除字数外，按下列各项逐句自检并就地改写（技法见 `moshu-setup/references/agent-references/dialogue-mastery.md`）——**这是常规收尾步，不依赖外部审查是否被 spawn**：① 机械对话（信息倾倒/问答式/句间无情绪承接）；② 角色当"科普嘴"（整段讲设定/原理，Gate G 同样管台词）；③ 说话不分场合（高压 beat 的玩笑/口头梗/插科打诨出戏）；④ 高位者长篇自证掉价（改成行动或一句压迫性回应）；⑤ 捧哏工具人（改成实际反应、吃瘪或行动）；⑥ 情绪水肿咆哮（压成短句、动作破坏或生理反馈）；⑦ 鹦鹉学舌复读；⑧ 生死场景嘴碎；⑨ 过早打断悬念。命中即改，改完再交付。

### 写完后文风自检（`文风库/文风.md` 存在 / 主会话传入 `style_profile_summary` 时必做，防续写文风漂移）

长篇续写到一定篇幅，正文容易渐渐漂离对标书文风——典型是越写越碎、句子被切成三五字、逗号/句号堆成「逗号结巴体」。每章写完后除字数外按下方核对一次，**漂了就当场拉回重写再交付**：

1. 取目标句长带——优先使用主会话本章传入的 `style_profile_summary`（对应文件见 `style_profile_path`）；否则读取 `文风库/文风.md` **有数值的「句长分布」字段**（短句<15字占比 / 中句 / 长句 / 平均句长，`confidence: high`）；都没有时以「原文锚点片段」语感为准。续写状态卡不存文风。
2. 粗测本章正文：标点（`。！？，、；`）分隔的句段是否大量不足 6 字、平均句段明显短于目标带、通篇碎句无呼吸感。
3. 命中即判**文风漂移**：按目标句长带把碎句合并成中长句、补回画面与连接、恢复叙述呼吸，重排后再交付。**词汇指纹核对**：正文高频副词若踩中文风标注的「过度副词警告」词（竟/忽然/仿佛/蓦然/不禁/猛地 等）——合并/替换，克制使用。**续写衔接的是剧情、不是上一章的句式节奏——文风以 `style_profile_summary` / `文风库/文风.md` / 原文锚点为准，不以可能已漂移的上一章为准。**

---

## 审查能力（附属，需用对抗性 prompt）

审查时任务是**找问题**，不是验证正确性，以最严苛的标准审视。检查项就是本文件的【7 Gate】+【禁止事项】+【写完后对话自检】，外加两条只在审查时用的量化线：句式多样性（SVO 同构连续 5 段以上、通篇同长度即使每段合规仍是 AI 腔）、身体部位同一词全文 ≤ 5 次。评分体系（五维评分 + 通用检查）见 `moshu-setup/references/agent-references/quality-checklist.md`。

被 moshu-review spawn 时，以其 prompt 内联的 rubric 与 AI 味摘要为准，本节只作兜底。
...
- **情绪词默认外化**：「悲伤」「愤怒」「恐惧」默认用身体状态替代，一处到位即可
...
| 约束维度 | 类型 | 与文风冲突时谁优先 |
|---|---|---|
| Gate A 禁用词 / `moshu-setup/references/agent-references/banned-words.md` | 硬 | banned-words 优先 |
