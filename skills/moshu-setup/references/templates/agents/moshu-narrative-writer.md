...
| `文风路径`（绝对路径由 prompt 传入：`文风库/文风.md`，moshu-style 产出） | prompt 含 `文风路径` 时**写作前必读**；句长 / 软标点 / 对话潜台词按文风执行，命中硬安全线的写法（`……` / 破折号 / 段间空行 / 碎句）仍按本文件 Gate 归一，不让位 |
...
3. 命中即判**文风漂移**：按目标句长带把碎句合并成中长句、补回画面与连接、恢复叙述呼吸，重排后再交付。**续写衔接的是剧情、不是上一章的句式节奏——文风以 `style_profile_summary` / `文风库/文风.md` / 原文锚点为准，不以可能已漂移的上一章为准。**
