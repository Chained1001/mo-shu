...
- **对照目标**（B53）：如收到的材料含虚拟对标.md，节奏/结构判断须对照其「节奏目标」与「结构要点」——
  不可纯凭感觉说"节奏偏慢"，须说"低于虚拟目标的每 N 章一个高潮"；
  如含理想书评.md 的结构化评分，商业判断须对照目标分数。

### 作者维度（技艺/新鲜度之眼）

- **新鲜度检查**：核心桥段/结构在已出版作品里见过类似的吗？
  举 2 个例子（作品名+桥段名）。举不出来说明什么（太平淡 or 太新没验证）？
- **对照参照**（B53）：如收到的材料含虚拟对标.md 的「结构要点」（中点/对手升级/伏笔密度模式），
...
- **对照基准**（B53）：如收到的材料含理想书评.md 的节奏目标（期待密度/释放频率），
