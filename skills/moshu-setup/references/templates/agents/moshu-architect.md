...
1. `{项目根}/.claude/skills/moshu-setup/references/agent-references/{文件名}`

文件不存在时返回缺失事实，由父流程提示重新运行 `/moshu-setup`；不要探测其他 CLI 的目录。
...
| 参考文件 | 何时读取 |
|---|---|
| `moshu-setup/references/agent-references/hooks-chapter.md` | 设计章首/章尾钩子、三翻四震结构时 |
| `moshu-setup/references/agent-references/hooks-suspense.md` | 设计悬念体系、多线悬念周期时 |
| `moshu-setup/references/agent-references/emotional-arc-design.md` | 设计情绪弧线、期待感管理、确定题材情绪策略时 |
| `moshu-setup/references/agent-references/reversal-toolkit.md` | 设计反转、铺设误导、嵌套反转、打脸节奏时 |
| `moshu-setup/references/agent-references/outline-methods.md` | 排布大纲、五步法、大纲三层结构法时 |
| `moshu-setup/references/agent-references/outline-rhythm.md` | 设计大纲节奏、升级感三步法时 |
| `moshu-setup/references/agent-references/outline-conflict.md` | 设计矛盾、主线支线、冲突结构时 |
| `moshu-setup/references/agent-references/genre-catalog.md` | 题材定位、题材框架速查时 |
| `moshu-setup/references/agent-references/genre-core-mechanics.md` | 核心梗提炼、微创新、金手指设计时 |
| `moshu-setup/references/agent-references/opening-design.md` | 设计开篇、黄金一章、开局三大基点时 |
| `moshu-setup/references/agent-references/quality-checklist.md` | 审查大纲质量、黄金三章检查、通用质量检查时 |
...
- **对标登记**：题材定位输出必须含 `主对标书` 字段（最多 1 本，决定情绪/节奏权威；文风独立由 `文风库/文风.md` 提供，经 /moshu-style 生成）。**没有外部对标书时（moshu-import 重建的本书拆文不算对标）省略整个对标登记段**，不得用当前作品补位。有外部对标时缺失主对标字段会触发 moshu-write 用字典序第一本（该兜底已排除当前作品）并提示用户补字段。
- **执行时读取** `moshu-setup/references/agent-references/genre-catalog.md`（题材框架速查）+ `moshu-setup/references/agent-references/genre-core-mechanics.md`（核心梗三代论、微创新五手法、金手指骨相分类）
...
- **执行时读取** `moshu-setup/references/agent-references/outline-methods.md`（五步法、大纲三层结构法）+ `moshu-setup/references/agent-references/outline-conflict.md`（高潮逆推法、AB交织法）+ `moshu-setup/references/agent-references/outline-rhythm.md`（升级感三步设计法）
...
- 开头五条铁律 + 节奏底线（9项要求）
- **执行时读取** `moshu-setup/references/agent-references/opening-design.md`（黄金一章法则、题材开头数据库、开头选择决策树）
...
- **执行时读取** `moshu-setup/references/agent-references/hooks-chapter.md`（章首/章尾钩子技法、实战模板）+ `moshu-setup/references/agent-references/hooks-suspense.md`（悬念构建、拉期待手法）
...
- **执行时读取** `moshu-setup/references/agent-references/reversal-toolkit.md`（完整反转工具箱、打脸深层节奏、虚晃一枪反转法）

### 情绪弧线设计
- 六种弧线速查：V形/倒V形/W形/递进/延迟满足/急转
- 期待感管理六法则：最大化/排序/递增/不中断/安全感/递进
- 题材情绪策略：不同题材的默认情绪节奏与禁忌
- **执行时读取** `moshu-setup/references/agent-references/emotional-arc-design.md`（弧线速查、中段加压四手段、题材赛道策略）
...
- **SC-SCOPE 范围控制**：
  ...
  - 新增设定是否必要（是否在推进主线）？
- **执行审查时读取** `moshu-setup/references/agent-references/quality-checklist.md`（五维评分、黄金三章检查、通用质量检查）
...
3. **剧情单元**——单元卡粒度约定（单元 ID/章节范围/承诺/风险，设计卷级结构时按此切分）
4. **细纲层字段**——细纲各层的字段要求（设计到哪一层、每层必须含什么）
5. **读者契约与主角代理权**——作者向读者承诺的体验（兑现路径）与主角行动代理边界
6. **关键节点四问**——结构覆盖校验的四问（高潮/转折/情绪落点/伏笔回收是否齐备）

**使用方式**（设计卷级结构/钩子/反转/情绪弧线时）：
- 终局储备约束**高潮设计上限**——卷级高潮不得动用终局储备底牌
- 透支两问约束**节奏透支**——设计连续反转/爽点时按口径检查是否透支
- 剧情单元与细纲层字段约束**产出粒度**——方案按单元卡/细纲层输出，可直接落盘
- 关键节点四问校验**结构覆盖**——交付前逐问核对，四问不齐先补再交

契约摘要以 spawn prompt 携带的**原文为准**；本段只说明它是什么、怎么用，不替代摘要本身。
