...
- **对标登记**：题材定位输出必须含 `主对标书` 字段（最多 1 本，决定 moshu-write 日更默认调用哪本文风）。**没有外部对标书时（moshu-import 重建的本书拆文不算对标）省略整个对标登记段**，不得用当前作品补位。有外部对标时缺失主对标字段会触发 moshu-write 用字典序第一本（该兜底已排除当前作品）并提示用户补字段。
