...
| 参考文件 | 何时读取 |
|---|---|
| `moshu-setup/references/agent-references/character-basics.md` | 设计角色（主角卡/配角卡/反派层级/动机链）时 |
| `moshu-setup/references/agent-references/character-design-methods.md` | 设计角色反差、深化人设、九维人设框架时 |
| `moshu-setup/references/agent-references/character-relations.md` | 设计角色关系类型、关系图时 |
| `moshu-setup/references/agent-references/dialogue-mastery.md` | 创作对话、设计潜台词、审查对话质量时 |


- **角色设计参考**：
  - 基础模板：项目内搜索 `moshu-setup/references/agent-references/character-basics.md`
    - 设计角色前：阅读"主角卡""配角卡""动机链"
    - 设计反派时：阅读"反派层级""反派建立四要素""反派性格确立四步法"
  - 深化方法：项目内搜索 `moshu-setup/references/agent-references/character-design-methods.md`
    - 设计角色前：阅读"三层标签反差人设法""九维人设框架"
    - 设计关系时：阅读"人设关联分层""以梗为中心塑造人设"
  - 关系设计：项目内搜索 `moshu-setup/references/agent-references/character-relations.md`
    - 设计关系时：阅读"人物关系类型"

- **对话创作参考**：项目内搜索 `moshu-setup/references/agent-references/dialogue-mastery.md`
...
设计角色时参照 `moshu-setup/references/agent-references/character-basics.md` 中的主角卡/配角卡模板：
...
参照 `moshu-setup/references/agent-references/dialogue-mastery.md` 中"人物语言差异化"的7维方法：
1. 口癖和惯用语：标志性用词
2. 说话节奏：长篇大论 vs 短句连击
...
参照 `moshu-setup/references/agent-references/character-basics.md` 中的动机链模型（起因→意图→约束→风险）：
...
参照 `moshu-setup/references/agent-references/character-design-methods.md` 中"九维人设框架"的成长弧线三阶段模型：
- 成长触发：什么事件打破现状
...
四种关系类型（参照 `moshu-setup/references/agent-references/character-relations.md`"人物关系类型"章节）：
...
参照 `moshu-setup/references/agent-references/dialogue-mastery.md` 中的核心方法：
...
审查前先阅读 `moshu-setup/references/agent-references/character-basics.md`"质量检查清单"章节，按维度逐项排查：
- **性格一致性**：角色在不同场景下的行为是否符合同一性格设定
- **关系一致性**：角色间的关系变化是否有迹可循、有无突然变化但缺乏铺垫
- **能力一致性**：角色实力/能力是否前后一致，有无战力崩坏
- **信息一致性**：角色知道什么/不知道什么是否前后一致

对话质量审查参照 `moshu-setup/references/agent-references/dialogue-mastery.md`"质量检查"三大自查项：
1. 是否存在大量信息都必须用对话来展示
2. 对话是否是问答式的一问一答
...
2. **不要让所有角色说话一个味**：如果遮住角色名后无法区分是谁在说话，说明差异化失败。必须用 `moshu-setup/references/agent-references/dialogue-mastery.md` 的7维差异化方法逐一检验。
