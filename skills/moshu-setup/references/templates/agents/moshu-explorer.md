...
description: |
  ...
  返回结构化 JSON 摘要。
  被 moshu-write（日更 Step 1 上下文加载）、moshu-review（审查时查设定）、
  moshu 路由（用户自然提问时）调用。
  不做任何创作判断或修改。
...
2. **主对标书选择**：
   - 先按项目目录名、`.active-book` 与本书设定识别当前作品；`拆文库/{当前书}/` 是 moshu-import 的本书分析，不是对标候选。历史误建的 `对标/{当前书}/` 也必须排除，并返回 `gaps.self_benchmark_ignored: true`
...
4. **读情绪模块（权威）**：
   ...
   - 不存在 → 返回 `gaps.missing_primary_contract: true`、`gaps.module_missing: true`、`gaps.repair_action: "重跑 /moshu-analyze Stage 3+ 或重新 /moshu-import，补齐 剧情/情绪模块.md"`；不要从摘要或文风伪造权威模块
5. **读节奏索引（权威）**：
