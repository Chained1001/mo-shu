...
1. 检查当前目录是否已部署过（存在 `.story-deployed`）
   ...
   - `agents_version` 大于 `25` → 当前 moshu-setup 比项目部署旧；停止以避免降级覆盖，提示先更新 mo-shu，不写任何部署文件
...
| `skills/moshu-setup/scripts/merge-claude-settings.py` | 部署时执行，不复制到项目 | moshu-setup helper | execute | 替换已知 moshu hook 注册、保留用户 hooks/顶层字段，v24→v25 迁移与重复执行幂等 |
