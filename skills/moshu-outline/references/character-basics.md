...
> 设计主角/配角/反派时加载。先看决策路由，选对模板，再按步骤操作。moshu-character-designer 主输出参考。

> **消费点（B103 刷）**：4.3 落盘卡模板
---
...
```
状态：计划（正文首次出现并提交追踪事务后改为 已转正；计划对象不得被审查当事实引用）
姓名：
性别：
角色定位：（一句话说清主角在故事中的功能）
...
对话 DNA（B59 五要素；有对标→建卡时从拆文库角色卡带入；无对标→留占位待填，不杜撰）：
- 口头禅功能位：（这句话在什么场合说、起什么作用）
- 句长习惯：（短促/绵长/混用）
- 语气词与标点：（如：句尾带"呗"、多用省略号）
- 回避话题：（什么绝口不提）
- 情绪外显：（情绪上来时的表达方式——冷嘲/爆发/沉默）
```
...
对话 DNA（B59 五要素；有对标→从拆文库角色卡带入；无对标→留占位待填，不杜撰）：
- 口头禅功能位：
- 句长习惯：（短促/绵长/混用）
- 语气词与标点：
- 回避话题：
...
- 经典反派一定有完整世界观和逻辑自洽的三观——同样的外部事件发生在不同人身上产生不同变化（外因与内因共振），不能只有外部事件、缺乏内在三观和内在逻辑
- **反派人设主角化**（换地图后原反派跟不上节奏 → 引入新反派脸谱化的问题）：把反派当主角去写；在"配角衬主角"基础上加"配角衬配角"；换地图的正确处理 = 用新地图衬托旧角色含金量，而非用旧角色无力衬托新地图难度
...
> 犯错尺度（可原谅 vs 不可原谅边界）唯一权威见 [character-design-methods.md](character-design-methods.md)「主角可以犯错但需把握尺度」——此处不重复。
...
设计完角色后，逐项检查（通用项——说话风格区分/可原谅范围/反派逻辑自洽/人设前后一致/代入感链条/安全感匹配——唯一权威见 [character-design-methods.md](character-design-methods.md)「质量检查清单」，此处不重复；以下为 basics 特有检查项）：
...
- [ ] 按层级表设计，篇幅与层级匹配
- [ ] 出场展示了实力或手段
- [ ] 至少赢主角一次（中等反派及以上）；没有降智送赢
- [ ] 大弧Boss有理念冲突（不只是利益冲突）；最终Boss从第一章就有伏笔
