...
**验收接入**：以上 `[BLOCK]` 项与 [output-contract.md](output-contract.md) 「structure_counts 数值校验」表共同构成「BLOCK 项扫描」的检查清单。SKILL.md「验收」段在 Stage 6 内容写完后、`stages_completed[6]` append 前调用本节。
- [ ] 节奏速报已包含
