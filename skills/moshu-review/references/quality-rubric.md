...
> 用途：当用户未指定番茄、起点等目标平台时，作为 `/moshu-review` 的默认小说内容评分标准。它评估的是**故事文本质量**，不是 skill/plugin 实现质量。
...
| 高潮构建 | 蓄能→假胜→崩解→交叉死磕→悬置收尾有层次 | 有爆点但蓄能或兑现不足 | 平铺直给、无代价、无兑现或情绪落空 |
| 关系进展 | 互动尺度匹配关系阶段，越界有铺垫 | 局部推进略快但可补证据 | 突然亲密/信任/敌对，角色关系跳变 |
| 伏笔状态 | 伏笔埋设、状态和回收路径可追踪 | 伏笔偏密/偏疏但不影响理解 | 伏笔与设定冲突、断线或造成主线理解混乱 |
| 信息差边界 | 对照信息差登记表，本章不泄露「读者已知=否」信息，出场角色知识边界未被突破 | 局部认知越界有铺垫可解释 | 提前泄露读者不该知道的信息，或角色知道了他不该知道的事 |
| 创意约束 | 金手指/优势使用在创意约束表边界内（有代价/有弱点/解法不被反复平推） | 局部使用略超边界但有后续代价铺垫 | 金手指万能化：无损使用、无代价、无弱点或核心问题被优势平推 |
