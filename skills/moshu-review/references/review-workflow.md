...
**数值正本优先级（B62）**：审查中发现数值类事实（境界/欠款/排名/资产等）矛盾时，仲裁序 = 追踪 state > 细纲 > 正文回忆——按此序裁决，不信正文里后写的数字。
...
- 信息差边界：对照 `追踪/信息差.md`（权威派生视图，批 3a；G\d{3,} 登记表）与 `设定/基本设定.md`（旧书题材定位.md）信息差登记表（如存在）——本章正文是否泄露「读者已知=否」的信息、出场角色的知识边界是否被突破（角色知道了他不该知道的事）；提前泄密按影响定 S2/S3。正文命中登记关键词的，可在报告标注"可标记兑现（作者确认）"。
- 创意约束：对照 `设定/基本设定.md`（旧书题材定位.md）创意约束表（如存在）——金手指/优势使用是否越界（无损使用、无代价、无弱点、同一解法被反复使用、核心问题被优势平推）；金手指万能化倾向按影响定 S2/S3。
...
4. **识别目标平台并加载 rubric**：
   - 优先使用用户显式指定的平台。
   - 其次读取项目文档里的 `目标平台` / `平台` 字段，例如 `设定/基本设定.md`（旧书题材定位.md）、`大纲/`、`拆文报告` 等。
