...
> 遇到此类（甜宠/喜剧/报应型），按「无反转」处理：走甜度递进或报应兑现，不强行设计反转。

---

## 嵌套反转

> **苏式问（嵌套设计前必答）**：你的外层反转和内层反转服务同一个叙事目的吗？如果拆掉内层，外层还成立吗？——答不出「同目的」或「拆掉内层外层仍成立」，说明嵌套只是堆反转，砍到单层。
...
> **苏式问（揭示前必答）**：这个反转读者回头看时会觉得"原来如此"还是"什么鬼"？如果是后者，缺少哪条线索的铺垫？——答不出具体缺哪条线索，按下方清单逐项回查（合理性/冲击力/公平性/节奏四组）。
