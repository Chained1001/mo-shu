...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - **模块召回、题材卡与文风召回**：
     ...
     - (d) **文风召回**：先直接读 `设定/文风.md`（不经 explorer）：含实质内容（去空白 ≥200 字，或含 句长 / 标点 / 对话 / 锚点 / 笔调 小节且小节内有可执行约束：比例 / 例句 / 禁止或偏好描述）则置 `custom_style=true`、进入「自定义文风模式」，它作权威风格基（句长 / 软标点 / 潜台词 / 情绪交替），对标 / 拆文 `文风.md` 降为参考（锚点 + 句长兜底）；空 / 仅空白 / 仅标题 / 占位 stub（待办 / 待补充 / ___）视为不存在。否则按「对标书路径查找」规则读 `{对标书路径}/文风.md`（路径优先 `{项目}/对标/{书名}/`，回退 `拆文库/{书名}/`），主对标书从 `设定/题材定位.md` 读 `主对标书` 字段。**文风阶段语义**：`文风.md` 存在即视为有文风（`文风阶段：表达层` 或 `完整` 均可）——表达层字段（句长带 / 标点 / 对话技法 / 锚点片段）照常消费；`文风阶段：表达层` 时「情绪交替」「可借鉴技巧」缺失不阻塞（情绪走 `剧情/情绪模块.md` 权威、节奏走 `剧情/节奏.md` 权威，可借鉴技巧是增强）；锚点全缺视为退化（`profile_degenerate`，按默认 Gates 写作）。**未进入自定义文风模式且**文风文件不存在 → **fail-fast 报错**：「对标书 X 缺少 文风.md。请用 `/moshu-analyze` 跑 Stage 1 停靠点生成表达层文风（或全量拆解后 Stage 6 补全），再 `/moshu-import` 同步。」不 inline 生成（自定义文风模式则不 fail-fast；情绪 / 节奏轴 `missing_primary_contract` 仍独立阻塞）
