...
2. **读取上下文**（按需选择；缺失时遵循各项及本文件「写前准备契约」的「缺失文件处理」，仅明确标为可选的非主产物跳过。可选快捷路径：如果项目已部署 moshu-explorer agent（检查 `.claude/agents/moshu-explorer.md` 是否存在），可 spawn `Agent(subagent_type: "moshu-explorer", prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章\n追踪状态：last_committed_chapter={check 的值}，state_revision={check 的值}")` 一次获取上下文）：
