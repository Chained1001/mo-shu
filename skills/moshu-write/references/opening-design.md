...
**激发事件四问**（来源：《小说的骨架》第 6 章，otherMaterials/referFile，2026-08-22 萃取）——用四问精确定义"切入点"（主角遭遇的第一个冲突/机遇）：

1. **什么是激发事件**：如同一道"门"——若主角能轻易抽身、回到从前的安稳生活，那就还没踏入那扇门。
2. **什么不是激发事件**：事件前的重要场景（如继承危机、权力交接）虽重要，但未颠覆主角世界的，不算。
3. **在何处**：书里约定落在全书四分之一处或之后，转译到中文网文即**开篇期口径**——开篇前几章内（黄金一章检查清单的窗口）。先让读者熟悉主角的日常与身份，再让激发事件打破日常，读者才感到事态紧急。
4. **好激发事件的四特质**：① 直接影响之后的情节（若没发生后续仍可上演，则不算）；② 制造冲突（变化越大冲突越激烈）；③ 抓住注意力（足够特别，不满足于平庸）；④ 引发动作（主角真正行动，带来不可逆变化）。

> 与「黄金一章检查清单」的衔接：黄金一章要求"从最精彩处写起、前 3 章内冲突起点"，本四问进一步回答"第一个冲突/机遇到底怎么定义、放在哪、有什么特质"——第 3 问的"开篇期"即黄金一章的窗口。

> 三问（准入）：谁在什么阶段调用——开书 Stage 2 核心设定/开篇设计时，用四问定义切入点；解决什么问题——"把日常切入或前戏误当激发事件、开篇没有明确的推动核心"；调用后可观察结果——开篇有明确的触发动作锚点，黄金一章"冲突起点"可核对。

**开篇缺失清单（B105 补——救猫咪 Ch4 :1329）**——开篇的另一半是「缺什么」：
- 列主角缺失 3-6 项（能力/关系/认知/身份任一维）——例：不会拒绝人（能力缺）/没有同盟（关系缺）/看错父亲（认知缺）。
- 每项缺失在开篇期**至少一次具象场景展示**（读者亲眼见过才算立住）。
- 这些缺失=定时炸弹——后文按升级台阶逐项回收（回收点=成长兑现点）。
- 检查问：**这项缺失读者见过吗？没见过的兑现=无感**（读者不记得你没收过的东西被收）。
