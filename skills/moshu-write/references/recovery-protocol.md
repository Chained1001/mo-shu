...
**特征**：对标权威文件缺失（`剧情/情绪模块.md`、`剧情/节奏.md`）或文风库缺失/不合规，或本项目主产物缺失。
...
| explorer 返回不完整 | `context_load` 结果缺项 | 回退到手动加载步骤（workflow-daily Stage 4-1 手动清单） |
...
| 场景 | 引用位置 |
|---|---|
| 单章写作失败处理 | `workflow-chapter.md` Stage 4-12「更新追踪」 |
| 日更批量失败处理 | `workflow-daily.md` Stage 4-2 事务提交 5 条 + 批末校验 |
