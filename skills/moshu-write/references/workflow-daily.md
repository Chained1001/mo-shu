...
> **日更准备步骤**：每章写作前 4 步——状态筛选 → 本章设定包 → 模块召回 (a)-(g)（含题材正文提示卡、文风、情绪模块/节奏、匹配章节）→ 指令确认，嵌入 Step 2 逐章循环（口径与 workflow-chapter.md 写前准备一致）。
>
> 写前资料清单/题材卡/文风召回/缺失处理/权威优先级一律按 `workflow-chapter.md` 写前准备 (a)-(g) 与「写前准备契约」执行，本文件不另立一套。读者契约/主角代理权/期待债/终局储备参 `reader-contract-and-progression.md`。
>
> **主对标书**：从 `设定/题材定位.md` 读 `主对标书`；指向当前作品时按缺失处理，缺失时按「对标书路径查找」规则（优先 `对标/`，为空回退 `拆文库/`）取字典序第一本并提示补字段（先排除同名或来源指向当前正文的 `对标/{当前书}/`）。
...
| 序号 | 文件 | 用途 | 如果不存在 |
|------|------|------|-----------|
| 1 | `tracking_commit.py check` | 校验唯一结构化 state 与全部派生视图，并取得最后提交章和修订号；不把完整 state 加入 prompt | state 缺失且新书尚无正文时执行初始化；已有正文则停止并要求由 `moshu-import` 重新导入为标准项目 |
...
| 6 | `追踪/角色状态/{角色名}.md` | 久别核心角色的派生当前快照；只按细纲涉及角色加载 | 缺失即视为派生视图损坏：先运行 `tracking_commit.py check`，再重跑产生当前状态的完整事务；已有正文却无 state 时重新 `moshu-import` |

`追踪/伏笔.md`、`追踪/时间线/`、`追踪/逐章记录/` 默认不整份读取。续写状态卡没有某个旧信息时，才按「旧信息查找步骤」定点查询。

**按需加载创作公式**：当写作中需要引用题材公式约束时（6 个长篇题材的结构骨架），加载 `references/genre-writing-formulas.md`。默认不加载，避免无条件加载约 116 行文件浪费 token。

### 续写状态卡与追踪事务

`追踪/上下文.md` 不是历史档案，而是当前语义检查点中的续写状态卡；顶层只能有以下 7 个栏目：`当前位置 / 长期约束 / 核心角色状态 / 活跃伏笔 / 近三章速记 / 下一章承诺 / 连贯性风险`。目标 8192 字节，硬上限 12288 字节。文风每章从 `文风库/文风.md` 读取（moshu-style 产出；缺失按交互提醒处理）；质量计数、普通待办、文件行数索引、参照章使用记录、去 AI 味统计都不进入续写状态卡。
...
**首次初始化**（仅 `_tracking-state.json` 缺失时）：无正文 → 构造 `last_chapter=0` 的 init 事务执行 `tracking_commit.py init`；已有正文 → 走 `/moshu-import` 的「旧追踪项目迁移」重建 `追踪/`（不重跑全书拆解，`init` 会把旧结构移入 `追踪/_旧追踪存档/`），本 workflow 不解析旧结构。

**事务边缘规则**（长期约束最多 6 条、退役必须显式声明、与项目 rules 的字段映射）统一见 [tracking-transaction.md](tracking-transaction.md)——每章提交事务前按该文件构造，本文件不重复。

> **编号与修订号**：执行 `tracking_commit.py check` 取 `last_committed_chapter + 1` 与 `state_revision`，核对 `上下文.md` 当前位置；逐章事务写 `expected_state_revision`，状态已变则重构。不要把完整 state 读进 prompt。
...
2. **逐章执行**：每章按 [workflow-chapter.md](workflow-chapter.md) 的「单章写作流程」步骤 1-13 完整走一遍（写前准备、正文执行、字数验证、钩子/爽点检查、元信息与禁用词扫描、追踪事务）。批量模式下再叠加下列日更专属动作：
   ...
   - **moshu-explorer 召回的 gaps 分支**（用 `benchmark_style_load` query_type 一次拿到 `{style_profile_path, style_profile_summary, selected_emotion_module, rhythm_reference, module_source_path, rhythm_source_path, matched_chapter_K, matched_chapter_techniques, anchor_excerpts, gaps}` 后按此分流）：
     ...
     - 若 `gaps.conflict` 或 `gaps.module_rhythm_conflict: true` → 意图确认必须说明冲突并按 `剧情/情绪模块.md` / `剧情/节奏.md` 的权威优先级执行；不得让文风覆盖情绪/节奏目标
     - 若 `gaps.profile_missing: true`（`文风库/文风.md` 缺失）→ 按 workflow-chapter (d) 交互提醒（/moshu-style 生成 / 跳过记录 / 自写约束）
     - 若 `gaps.profile_degenerate: true`（文风不可用：锚点缺失/`文风可用：否`） → 提示建议重跑 /moshu-style；用户确认后跳过文风、回到默认 Gates 写作
...
批末再对本批全部落盘正文整体跑一遍 workflow-chapter.md「质量检查」的确定性收尾（`check-ai-patterns.js` → `check-outline-copy.js` → `normalize-punctuation.js`，另加 `check-degeneration.js` 退化防护），确认逐章清零后没有回潮。
