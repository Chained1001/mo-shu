...
2. **逐章执行**：每章按 [workflow-chapter.md](workflow-chapter.md) 的「单章写作流程」步骤 1-13 完整走一遍（写前准备、正文执行、字数验证、钩子/爽点检查、元信息与禁用词扫描、追踪事务）。批量模式下再叠加下列日更专属动作：
   ...
   - 每章写完后**立即提交一次追踪事务**：
     ...
     9. **章后对照（细纲落地核对）**：对照本章细纲「情节安排」逐点核对落地情况（每条引用正文位置）——已写（landed）无动作；未写（missed）影响后续 → 进 `next_chapter_commitments` 或 `continuity_risks`，不影响（可舍弃过渡点）→ 在细纲该点标注 `<!-- 未落地: 原因 -->`；偏离（deviated）合理（正文更好）→ 修订细纲该点并标注，不合理 → 下章补写或改细纲；正文出现细纲未安排的事件 → 归类：延续线索登记进 `foreshadow`/`timeline_events`，一次性事件不登记。正文是最终真相，细纲跟随正文修订（已写区间属锁定，修订需用户确认）。
