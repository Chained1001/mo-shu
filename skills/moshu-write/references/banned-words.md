...
| ★★★★ | 名词性独语句（感知碎片句，无主谓）| "冷。铁锈和旧木头的味道。" | 补全成完整句，感知挂到角色动作上："风灌进来。他打了个寒颤，铁锈和旧木头的味道直往鼻子里钻" |
| ★★★★ | 群像反应："全场震惊" / "所有人都倒吸一口凉气" / "在场的人面面相觑" | "在场所有人都惊呆了" | 挑一两个具体角色写个体反应（老陈的烟烫了裤子都没察觉），不写集体动作 |
| ★★★ | "（动词）了一下" | "抖了一下""看了一眼""愣了一下" | 换具体动词："他一抖""他扫了一眼"；"了一下"只留节奏必需处，成串出现必改 |
...
| ★★★☆ | 翻案腔变体："与其说A不如说B"/"看似A实则B"/"你以为A其实B"/"A不重要，重要的是B"/"回头才发现" | "你以为他赢了，其实他输得更彻底" | 判断从正面直接下 |

> 凡命中 ★★★★★ 一处即视为重度AI味的强证据；★★★★ 命中 ≥2 处即触发中度复扫。

确定性 AI 句式检测的 `formulaic-parallelism` 规则还会提示「至于X不X，怎么X」和同动词「不V A，不V B」。这两类可能是功能性口语，因此只做 advisory；Gate B 必须连同台词读语境复核，若只是复述细纲/前文就压成一次判断，不能因 hook 豁免台词而跳过。

**标点**：正文（含叙述和对话）禁用破折号 `——`/`—`、双连字符 `--` 和省略号停顿，改用句号、逗号、短句或动作断句；不设置对话破折号例外。古言/日式的「」引号不在此列。
...
### 模型路标词（叙述中替换，台词可保留）
说白了、说穿了、说到底、先说结论、值得注意的是、还有一层、从某种意义上说（叙述里当洞察路标时删掉）
...
| 动词名词化/提示性冒号："完成了对…的优化"/"一句话总结：" | "完成了对流程的优化" | 写回直接动词；冒号只留引直接原话 |
...
**套词密度优先处理**：确定性 AI 句式检测报 `cliche-density-tic` 时，说明禁用词不是零星误用，而是聚成了模板腔。处理顺序不是同义词替换，而是先删抽象总结，再把情绪/判断落到角色当下可见的动作、物件、对话和具体后果。
