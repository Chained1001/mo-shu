...
> 这三个 Phase 每本书只跑一次（开新卷时回 Phase 2/3 增量补充；先读 `大纲/卷复盘_第X卷.md`（如有）的下卷方向候选，作者已确认的直接作新卷起点）。日更续写与单章写作不需要本文件，
...
如果用户给的不是方向而是**模糊灵感**（画面/脑洞/感觉，说不清题材卖点）：
先做**灵感种子收敛**（`references/idea-seed.md`，产物落 `{项目根}/灵感种子.md`），Logline 作后续方向提问与 Phase 2 设定的一致性尺度。有方向 → 下方匹配；仍无 → 常规提问。
...
3. 有同题材或弱相关候选 → 用 AskUserQuestion 推荐（列候选书 +「都不用」+「另拆一本」），选定主对标 1 本（走情绪/节奏/正文；文风独立由文风库提供）。选定后写入 `设定/题材定位.md`「对标登记」的 `主对标书`（Phase 2 落文件，此处先记选择），并按 SKILL.md「路径与术语约定」的「首次引用对标书」规则把选中书从 `拆文库/{书}/` 复制到 `对标/{书}/`（复制清单不含 文风.md——文风独立由 `文风库/文风.md` 提供）。登记后若文风库尚无该书文风，询问用户是否现在用 /moshu-style 学习（可跳过，写第一章时缺失会再次提醒）。
...
- 力量体系：{如果有，简单概括}
- 社会结构：{影响故事的关键设定}
- 信息生态（可选）：{信息速度（大新闻多久传到普通人）/ 知识载体（知识在哪、谁掌管）/ 信息壁垒（谁垄断、封锁什么、手段）/ 传播失真（消息如何变味、被谁利用）}
- 货币与经济（可选）：{钱怎么来/怎么花/什么值钱；影响剧情动机与冲突的经济事实}
- 规则与禁忌（可选）：{世界运行的硬规则与打破后果；规则怪谈/系统文/能力设定必填}
- 资源与限制（可选）：{稀缺资源、能力代价、行动边界；防"金手指万能化"的事实约束}
- 术语表（可选）：{专有名词统一释义；世界观术语首次出现时保证可查，正文使用一致}

> 世界观骨架只记录**长期有效**的世界事实（v7 设定边界）；章节内的临时状态、当前关系变化、开放线索不进世界观文件，交给 `追踪/`。
...
- **设定/题材正文提示卡.md**：从 `设定/题材定位.md` + `references/genre-prose-cards.md`（索引）+ `references/genre-prose-cards/`（单题材正文卡目录，按题材分类优先）+ `references/style-genre-modules.md`（通用流派补充）抽取本书正文层题材卡，只写题材边界、核心逻辑、读者期待、核心爽点/情绪、正文落点、前中后期打法、节奏密度、场景颗粒、禁止漂移；不写通用格式规则，不覆盖 `文风库/文风.md`
...
> **对标节奏回流（有主对标书时，卷纲定稿前做一次）**：读主对标 `对标/{书}/剧情/节奏.md`；缺失时设置 `missing_primary_contract: true` 并停止，提示重跑 `/moshu-analyze` Stage 3+ 或重新 `/moshu-import`，不得用章节摘要、故事线或拆文报告代替。文件存在时，按 references/outline-structure-theory.md「对标节奏迁移」把对标的一级结构关键点（1/4·中点·3/4）换素材排进本卷卷纲「对标结构坐标」——选段以主对标剧情单元为单位，按「类型/桥段标签」圈同类（详见该节步骤 1），选中的剧情单元写入剧情单元卡「对标剧情参照」。若尚未登记主对标书但 `拆文库/` 有同题材候选，先回 Phase 1「对标发现」登记再排节奏；确无对标书则按八节点占比自排。
...
默认分批建纲：先建**首批细纲（默认 5 章，上限 10 章）**后**停靠**，报告"已可开始写第1章/日更"；只有用户明确要求写正文时才进入 Phase 4。滚动补纲按编号规则执行（见 references/outline-structure-theory.md「按剧情批出细纲」）。**大纲方法取用走 [outline-methods.md](outline-methods.md)「决策路由」总入口**（按建纲任务加载对应文件与节）：
...
5. **批末交叉核对**（每批建完必做，防卷纲↔细纲矛盾）：① 卷纲三类表（伏笔/悬念/感情线）的埋设章 vs 本批细纲「本章禁止提前释放」——冲突时**以细纲执行口径为准**修正卷纲表（卷纲是规划、细纲是执行，执行口径优先；测试实测：卷纲写"第1章埋系统来源激活语"而细纲禁止提前释放，同卷自相矛盾）；② 时间口径——细纲「时间锚点」vs 章尾钩子/任务面板/对话框里的时间表述（"第 N 天" vs "限时 X 小时"）必须一致；③ 章尾钩子承诺 vs 下一章开头承接（钩子承诺的兑现点要在后续细纲可见）。
...
- 信息留白（如适用）：{本章哪处**只写现象不写因果/动机**——正文在此位置只呈现可观察行为；真相备忘与最早揭露章：{作者侧记录真相，第 N 章才揭露}}——信息差/悬念/伏笔类章节必填，纯推进章可省
