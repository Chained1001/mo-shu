...
> **最近更新（v2.4.0）**：**创作-评审-采风闭环**——评审 Agent（moshu-evaluator，三维度只读·停靠屏例行）+ 多维采风触发（Stage 2-6 创作瓶颈信号+分语境融合）+ 方法论苏格拉底化（设计问句替代清单·填空式模板）+ Stage 命名标准化 + 机检格式容错 + 冷热分离性能优化 + 审计法 v1.6（填充测试/标签消费检出/版本散射工具化）+ 开发标准 v1.2 + agents_version 34（8 agent）。**v2.2-2.3 主体**：构建管线五层（骨架八列/势力场/暗线层次/舞台演进/支线配角高光）+ 三书逆向萃取 + 大纲机检 + 采风技能 agent 化 + 17 批迭代。更早版本变更见 [CHANGELOG.md](CHANGELOG.md)。
...
│       ├── 技法总结.md      # 拆书 Stage 6 产出（情绪交替/可借鉴技巧/分层建议）
