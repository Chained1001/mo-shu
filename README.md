...
> **最近更新（v2.3.6）**：采风 agent 化（壳芯架构+CF 需求票据+五类七源+融合四步）+ 舞台/暗线层次/情感低压三层补强 + 版本 bump 确定性脚本 + 开发标准入库 + 安装/部署体验修复（deploy.py 过期常量根治、安装引导三步走、版本展示）+ 大纲机检 check_outline.py。**v2.2-2.3 主体**：构建管线五层齐备（骨架六要素与势力场/三书逆向萃取/大纲机检/采风技能/支线暗线兑现）+ 十批建设+架构审计。更早版本变更见 [CHANGELOG.md](CHANGELOG.md)。
