...
> **最近更新（v2.6.0）**：成品标尺收口（理想书评+虚拟对标→核心设定标尺段）+ 写作模式重构（逐章写作 K 参数统一车道/A 段分级/改稿两路）+ deslop 收编与 moshu 路由/Dashboard 退役（12→10 技能）+ 审计四期整改（run-regressions 一键回归/盲区防假绿）+ 全仓瘦身（规格归档/方法论合并）；agents_version 47
