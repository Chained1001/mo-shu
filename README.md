...
> **最近更新（v2.5.3）**：采风接线与竞态修复——源优先级表 v2（搜狗 web 限流实测→搜狗微信+360 双 fallback/六内容源/禁用名单更新）+ spawn prompt 必含条款九条逐条搬入 + 轮 3/volume 采风前置检查（文件系统证据绑定必经步）+ spawn 构造纪律入开发标准；agents_version 45
