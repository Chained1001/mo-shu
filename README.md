...
> **最近更新（v2.5.1）**：**实测一期修复+全仓瘦身**——skeleton-template 四层十节（群像/钥匙列/卷间驱动链/承诺兑现时点/核心角色五件套[动机个人化]/因果前因注）+四项方法论萃取（金手指三定则/读者心理学/提纲动机链/人物构建学）+入口前置三查+卷纲四字段+台账→创作进度改名+采风模板 v2+轮 3 预填路径+轮 1 参考速写确认+例证≠需求入宪+步骤告知义务+全仓注意力优化（施工会话 6.5 倍减负+AGENTS.md 36% 减+agent-references 死库存清理）；agents_version 45
