...
> **最近更新（v2.0.0）**：构建迭代法（六步五面书驱构建：雪花法基底+设定/情绪面+对标双路+台账v2+完美评述/浮现记录）+ 写作三遍法（快写/读者重读/技艺打磨+上下文隔离）+ 知识策展管道（《小说的骨架》五条萃取）+ 治理收口（审计法v1.3+AGENTS自包含+两种各一份）+ 3 新守卫（reference-closure/route-write/story-numbers 扩展）。更早版本变更见 [CHANGELOG.md](CHANGELOG.md)。
