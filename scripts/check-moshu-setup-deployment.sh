#!/bin/bash
# check-moshu-setup-deployment.sh — moshu-setup deployment/runtime regression checks
...
SKILL_DIR="$REPO_ROOT/skills/moshu-setup"
...
copy_agent_refs() {
  local root="$1"
  mkdir -p "$root/.claude/skills/moshu-setup/references"
  cp -R "$AGENT_REFS_DIR" "$root/.claude/skills/moshu-setup/references/agent-references"
}
...
setup_git_repo() {
  local root="$1"
  git -C "$root" init -q
  git -C "$root" config user.email moshu-setup@example.invalid
  git -C "$root" config user.name moshu-setup-test
}
...
mkdir -p "$stale_previous_root/.claude/skills/moshu-setup/references/agent-references"
...
mkdir -p "$newer_project_root/.claude/skills/moshu-setup/references/agent-references"
...
mkdir -p "$mixed_version_root/.claude/skills/moshu-setup/references/agent-references"
setup_git_repo "$mixed_version_root"
copy_hooks "$mixed_version_root"
touch "$mixed_version_root/.claude/skills/moshu-setup/references/agent-references/dummy.md"
...
[ ! -d "$short_root/moshu/追踪" ] || fail "session-end created 追踪/ for short project even with STORY_SESSION_LOG=1"
...
printf '{"schema_version":8,"state_revision":0,"last_committed_chapter":1}\n' > "$cont_root/book/追踪/_tracking-state.json"
...
# 负向：追踪跟上 + 标题不重复不报
printf '{"schema_version":8,"state_revision":0,"last_committed_chapter":2}\n' > "$cont_root/book/追踪/_tracking-state.json"
...
# 的文档完整性断言——那种改一个词就红、测的是措辞不是行为，已随 check-moshu-write-contract.sh
...
assert_grep "agents_version.*小于 \`$CURRENT_AGENTS_VERSION\`|版本 < $CURRENT_AGENTS_VERSION" "$SKILL_DIR/SKILL.md" "moshu-setup redeploy branch must treat agents_version $PREVIOUS_AGENTS_VERSION as stale"
assert_grep "等于 $CURRENT_AGENTS_VERSION" "$SKILL_DIR/SKILL.md" "moshu-setup redeploy branch must confirm redeploy on equal agents_version（三分支三态锚点，v2.3.7 复核判因补齐）"
assert_grep "agents_version.*大于 \`$CURRENT_AGENTS_VERSION\`" "$SKILL_DIR/SKILL.md" "moshu-setup must stop before downgrading a newer deployment"
assert_grep 'Notice: agents bundle 版本不匹配' "$REPO_ROOT/skills/moshu-review/SKILL.md" "moshu-review must surface an agents_version mismatch"
...
grep -Eq 'missing_primary_contract' "$REPO_ROOT/skills/moshu-write/SKILL.md" "$REPO_ROOT/skills/moshu-write/references/chapter-core.md" \
  || fail "moshu-write must not silently fallback for missing primary artifacts (SKILL.md / workflow-daily.md)"
...
assert_grep '不用.*……|不使用.*……|不保留.*……|不残留.*……' "$SKILL_DIR/references/templates/agents/moshu-narrative-writer.md" "moshu-narrative-writer must reject ellipsis pause punctuation"
assert_grep '不用.*——|不使用.*——|不保留.*——|不残留.*——' "$SKILL_DIR/references/templates/agents/moshu-narrative-writer.md" "moshu-narrative-writer must reject dialogue dash exception"
...
assert_grep '禁止高置信否定铺垫后再肯定翻转|禁止高置信否定翻转句式' "$SKILL_DIR/references/templates/agents/moshu-narrative-writer.md" "moshu-narrative-writer must hard-ban high-confidence not-then-is flips"
assert_grep '跨段.*不是A / 也不是B / 只是C.*(只作语义复核|advisory)' "$SKILL_DIR/references/templates/agents/moshu-narrative-writer.md" "moshu-narrative-writer must treat cross-paragraph negation as advisory"
assert_grep '承担辩解、悬念排除或情绪递进时可保留|承担辩解、悬念排除、情绪递进等功能时可保留' "$SKILL_DIR/references/templates/agents/moshu-narrative-writer.md" "moshu-narrative-writer must preserve functional cross-paragraph negation"
assert_grep '至于X不X，怎么X' "$SKILL_DIR/references/templates/agents/moshu-narrative-writer.md" "moshu-narrative-writer must review formulaic dialogue too"
assert_grep 'check-ai-patterns\.js --check' "$SKILL_DIR/references/templates/agents/moshu-narrative-writer.md" "moshu-narrative-writer must require detector rescan handoff"
assert_grep '裸调用.*不得自动进入正文写作|不得自动进入正文写作.*裸调用' "$REPO_ROOT/skills/moshu-write/SKILL.md" "moshu-write bare invocation must not auto-write prose"
assert_grep '不得把已有项目默认为日更 3 章|默认为日更 3 章' "$REPO_ROOT/skills/moshu-write/SKILL.md" "moshu-write must not default existing projects to daily 3 chapters on bare invocation"
assert_grep '默认停在细纲交付|默认停靠.*Stage 1-3' "$REPO_ROOT/skills/moshu-write/SKILL.md" "moshu-write opening flow must stop after outline by default"
assert_grep '本轮 K（最多 3 章）后必须进入 Stage 4-D3/4-D4 收尾并停止|最多 3 章.*收尾并停止' "$REPO_ROOT/skills/moshu-write/references/workflow-daily.md" "daily workflow must stop after bounded batch"
assert_grep '细纲边界|outline_underfilled|不得自造剧情' "$SKILL_DIR/references/templates/agents/moshu-narrative-writer.md" "moshu-narrative-writer must enforce outline boundary and report outline_underfilled"
assert_grep '导入续写入口顺序|推荐顺序.*moshu-setup' "$REPO_ROOT/skills/moshu-import/SKILL.md" "moshu-import must answer setup-vs-import order before asking for source"
...
printf '{"schema_version":8,"state_revision":0,"last_committed_chapter":200}\n' > "$guard_root/book/追踪/_tracking-state.json"
...
[ "$(run_guard 'docs/正文.md')" = "0" ] || fail "guard wrongly blocked a non-moshu 正文.md (no 设定.md signal)"
# 已存在正文 -> 放行（续写/改稿/去AI味）
