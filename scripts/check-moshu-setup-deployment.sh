...
assert_grep 'agents_version.*小于 `25`|版本 < 25' "$SKILL_DIR/SKILL.md" "moshu-setup redeploy branch must treat agents_version 24 as stale"
assert_grep 'agents_version.*大于 `25`' "$SKILL_DIR/SKILL.md" "moshu-setup must stop before downgrading a newer deployment"
...
assert_grep '大于 25 时额外提示先更新 mo-shu' "$REPO_ROOT/skills/moshu-review/SKILL.md" "moshu-review must tell newer deployments to update the package first"
assert_grep '^version:[[:space:]]*1\.2\.7$' "$SKILL_FILE" "moshu-setup frontmatter must match the deployed setup version"
...
assert_grep '默认停在细纲交付|默认停靠.*Phase 1→3' "$REPO_ROOT/skills/moshu-write/SKILL.md" "moshu-write opening flow must stop after outline by default"
assert_grep '本轮 K（最多 3 章）后必须进入 Step 3/4 收尾并停止|最多 3 章.*收尾并停止' "$REPO_ROOT/skills/moshu-write/references/workflow-daily.md" "daily workflow must stop after bounded batch"
