...
| `test-check-outline.py` | 大纲机检回归：合规大纲 0 / 占比·中点·字数·悬空 F·删节 各 blocking 1 / 旧结构版本兼容降级 0 / 缺文件 2。对应运行时脚本 `skills/moshu-outline/scripts/check_outline.py`（blocking+candidate 两列，候选永不拦截；退出码 0=通过[含仅 candidate]/1=blocking 违规/2=参数或读文件错误[缺/空/坏]） | CI（调 moshu-outline 的 check_outline.py）；改 check_outline.py 后 | 大纲机检回归 | 未登记-待体检 |
