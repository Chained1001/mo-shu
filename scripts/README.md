...
| `test-design-fingerprints.py` | 设定指纹三子命令回归：record 幂等（两次哈希同）/ diff 反查正确性（stem+路径双匹配命中 [1,3]、无引用空数组、删除→missing）/ 读失败三分类（registry 缺失自动建档+baseline_created、设定目录缺失 exit 1）/ update 刷新后 diff 清空 | CI（调 moshu-build 的 design_fingerprints.py） | B61 指纹影响面 | 2026-08-28（B61 批内红绿：无引用 fixture 首版未排除他章引用，修正后绿） |
