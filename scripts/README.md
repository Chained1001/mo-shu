...
| `check-capability-wiring.sh` + `capability-wiring.json` + `check-capability-wiring.py` | 能力接线守卫：每个确定性能力的全部 consumer 文件必须含调用点标记（锁 producer→consumer 链通不通）。与 behavior-contracts 分工：本表锁「链通不通」，契约锁「话在不在」。新增能力先登记本表，删除能力同步清表 | CI；新增/删除确定性能力或改流程文档调用点后 |
...
| `test-capability-wiring.py` | 能力接线守卫回归：正向（真仓库全部接线）+ 反向（fixture 破坏调用点标记必须失败且指向能力 id 与文件） | CI（调 check-capability-wiring） |
| `test-reference-closure.py` | 引用闭包守卫回归（方案 A 资产宇宙）：正向（本域存在→过 / 资产宇宙内本域缺失→违规含文件名 / 白名单内→过）/ 反向（移除白名单→违规）/ 非资产宇宙运行态产物→忽略（⑤锁定方案 A 语义） | CI（调 check-reference-closure） |
...
| `test-bump-agents-version.py` | 版本 bump 回归：六类文件全覆盖（SKILL 反引号+无反引号/current-contract/session-start.sh/deploy-manual/UPGRADING 版本头）+ `--setup-version` 独立轨 6 处覆盖（含 deploy.py 常量+CLI 帮助）/历史条目不动/守卫红回滚还原（agents 或 setup 任一失败→全部回滚）。对应 `scripts/bump-agents-version.py`（预览 diff+`--confirm`+三守卫+失败回滚；退出码 0=完成/1=守卫红已回滚/2=参数或文件错误）。用法示例：`python scripts/bump-agents-version.py 34`（仅 agents_version）；`python scripts/bump-agents-version.py 33 --setup-version 1.5.1 1.6.0`（仅 setup_skill_version，独立轨） | CI；改 bump-agents-version.py 后 |
