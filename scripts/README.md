...
这些是开发本仓库（skill 套件本体）用的**守卫 / 测试 / 代码生成**脚本，**不是** skill 运行时脚本（运行时脚本在各 skill 自己的 `scripts/` 下，如 `moshu-write/scripts/check-ai-patterns.js`，跨 skill 字节同步）。
...
| `check-host-neutrality.sh` + `check-host-neutrality.py` | 宿主中性守卫（B76.6）：skills/ 除 moshu-setup 适配面外禁 `.claude`/`CLAUDE_PROJECT_DIR` 字面量（md/sh/py/js/json/mjs 六后缀）——宿主布局知识收敛于单一适配面（12-Factor III/适配器模式），运行时用「已部署→spawn→fallback」语义替代宿主路径探测 | CI；新增运行时宿主引用或改适配面后 | B76.6 宿主适配面收口 | 2026-08-29（B76.6 批内红绿：违规 fixture 红+白名单绿） |
...
| `test-style-profile.py` | 文风画像脚本回归：句长分布/段落节奏/标点密度谱/对话比/bigram/虚词密度精确值（合成 fixture）、读失败三分类、<800 字 warning、compare 相对差 | CI（调 moshu-style style_profile） | 文风画像确定性层 | 2026-08-26（B49：style 走查预习——确定性层薄（AI 手搓 one-liner）+2509.14543 分布锚依据） |
...
| `test-impact-scan.py` | 构建资产修订影响分析回归：正向（关键词命中未写细纲+已写正文+追踪三处）+ 反向（干净关键词三清单空）+ 无 state 退出 2 且引导 | CI（调 moshu-volume 的 impact_scan.py） | 影响扫描回归 | 未登记-待体检 |
| `test-design-fingerprints.py` | 设定指纹三子命令回归：record 幂等（两次哈希同）/ diff 反查正确性（stem+路径双匹配命中 [1,3]、无引用空数组、删除→missing）/ 读失败三分类（registry 缺失自动建档+baseline_created、设定目录缺失 exit 1）/ update 刷新后 diff 清空 | CI（调 moshu-volume 的 design_fingerprints.py） | B61 指纹影响面 | 2026-08-28（B61 批内红绿：无引用 fixture 首版未排除他章引用，修正后绿） |
| `test-host-neutrality.sh` | 宿主中性守卫回归（B76.6 三用例）：本仓全绿 / 违规 fixture 红 / 白名单（moshu-setup 适配面）绿 | CI（调 check-host-neutrality.sh）；改守卫或新增运行时宿主引用后 | B76.6 宿主适配面收口 | 2026-08-29（B76.6 批内红绿：fixture 隔离+wrapper 传参两处判因修正后绿） |
| `test-check-outline.py` | 大纲机检回归（B107 十三列）：十三列合规 0 / 占比·中点·字数·悬空 F·删节 各 blocking 1 / OLD10+十列+旧结构 三层降级 / 新列「—」占位 candidate / 暗线表+反转谱表两子节与主要人物节 blocking / 缺文件 2。对应运行时脚本 `skills/moshu-outline/scripts/check_outline.py`（blocking+candidate 两列，候选永不拦截；退出码 0=通过[含仅 candidate]/1=blocking 违规/2=参数或读文件错误[缺/空/坏]） | CI（调 moshu-outline 的 check_outline.py）；改 check_outline.py 后 | 大纲机检回归 | 未登记-待体检 |
| `test-pace-meter.py` | 推进度仪表回归（B68b）：13 用例——正常节奏无候选 / 规格 fixture 背离候选（剧情 75% vs 卷 10%，含预计收卷章数）/ pp 单条件（已消耗 0 → 收卷章数无法估算）与 50pp 恰等边界 / 缺大纲.md·缺卷纲·内容坏三分类降级 / 编号缺号 [10] / CLI 计划与阈值覆盖 / 无场景表 null / 千章边界（999/1000/1001 数值序，'1000'<'999' 字典序陷阱）/ 单元卡行格式样例双形态（workflow-build 钉源「单元 U{NN}｜章节范围：第{N}-{M} 章」+ 字段行）/ skipped_no_plan / 缺目录退出 1。对应运行时脚本 `skills/moshu-write/scripts/pace_meter.py`（shared-assets 跨技能副本 build 同步；呈报工具恒退出码 0：卷进度 vs 剧情进度背离候选+细纲编号连续性，候选永不拦截） | CI（调 moshu-write 的 pace_meter.py）；改 pace_meter.py 后 | B68 推进度仪表 | 2026-08-28（B68b 批内红绿：函数重复定义/场景表表格形态解析/末卷行回推口径三处判因修正后绿） |
