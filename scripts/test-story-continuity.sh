...
make_book() {
  ...
  printf 'c\n' > "$root/某书/设定/力量.md"
  printf '卷纲\n' > "$root/某书/大纲/卷纲.md"
  printf '%s\n' '{"schema_version":8,"state_revision":0,"last_committed_chapter":0}' > "$root/某书/追踪/_tracking-state.json"
...
for kind in mismatch malformed missing; do
  ...
  case "$kind" in
    mismatch) printf '%s\n' '{"schema_version":8,"state_revision":1,"last_committed_chapter":0}' > "$T_META/某书/追踪/_tracking-state.json" ;;
...
printf '%s\n' '{"schema_version":8,"state_revision":0,"last_committed_chapter":2}' > "$T2/某书/追踪/_tracking-state.json"
...
printf '%s\n' '{"schema_version":8,"state_revision":0,"last_committed_chapter":2}' > "$T3/某书/追踪/_tracking-state.json"
...
sleep 1
printf '%s\n' '{"schema_version":8,"state_revision":0,"last_committed_chapter":55,"foreshadow":{"F001":{"id":"F001","summary":"神秘信件","planted_chapter":1,"planned_resolution_chapter":null,"status":"已埋","importance":"高","updated_chapter":1}}}' > "$T4/某书/追踪/_tracking-state.json"
...
# ⑦ 旧 schema 拒绝：schema_version 落后于工具常量 → 拒绝并要求重新导入（B58b CI 修正补，守护 schema 漂移）
T5="$(mktemp -d)"; make_book "$T5"
printf '# 第1章 开端
正文。
' > "$T5/某书/正文/第001章_开端.md"
printf '%s
' '{"schema_version":5,"state_revision":0,"last_committed_chapter":0}' > "$T5/某书/追踪/_tracking-state.json"
printf '%s
' '> 状态修订：0' '上下文' > "$T5/某书/追踪/上下文.md"
out="$(run "$T5")"
printf '%s' "$out" | grep -q '不是当前 schema_version' || { echo "FAIL: 旧 schema 拒绝未触发"; echo "$out" >&2; fails=$((fails+1)); }
rm -rf "$T5"
