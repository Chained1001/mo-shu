...
make_book() {
  ...
  printf 'c\n' > "$root/某书/设定/力量.md"
  printf '卷纲\n' > "$root/某书/大纲/卷纲.md"
  printf '%s\n' '{"schema_version":6,"state_revision":0,"last_committed_chapter":0}' > "$root/某书/追踪/_tracking-state.json"
...
for kind in mismatch malformed missing; do
  ...
  case "$kind" in
    mismatch) printf '%s\n' '{"schema_version":6,"state_revision":1,"last_committed_chapter":0}' > "$T_META/某书/追踪/_tracking-state.json" ;;
...
printf '%s\n' '{"schema_version":6,"state_revision":0,"last_committed_chapter":2}' > "$T2/某书/追踪/_tracking-state.json"
...
printf '%s\n' '{"schema_version":6,"state_revision":0,"last_committed_chapter":2}' > "$T3/某书/追踪/_tracking-state.json"
...
sleep 1
printf '%s\n' '{"schema_version":6,"state_revision":0,"last_committed_chapter":55,"foreshadow":{"F001":{"id":"F001","summary":"神秘信件","planted_chapter":1,"planned_resolution_chapter":null,"status":"已埋","importance":"高","updated_chapter":1}}}' > "$T4/某书/追踪/_tracking-state.json"
