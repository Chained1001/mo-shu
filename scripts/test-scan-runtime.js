#!/usr/bin/env node
// 守护对象：CDP argv/JSON 契约与 scraper 无副作用 import。禁：断言实现细节/真实上游/脆弱快照（scripts/README.md 测试纪律）。
"use strict";
...
function testJjwxcDetailFailureIsolation() {
  ...
  for (const content of run.contents) {
    assert.match(content, /数据质量：\[存在问题\]/);
    assert.match(content, /问题摘要：.*详情解析异常\/登录态缺失/);
  ...
  assert.match(partial.contents[0], /问题摘要：.*部分详情缺失/);
}
...
function testFanqiePureFunctions() {
  ...
  const longDesc = fanqie.cleanDesc("甲".repeat(110));
  assert(longDesc.length <= 103, `简介须截断到 100 字+...，实际 ${longDesc.length}`);
  assert(longDesc.endsWith("..."), "截断后应以 ... 结尾");
  assert.strictEqual(fanqie.cleanDesc("平凡简介"), "平凡简介");
  assert.strictEqual(fanqie.fmtReads(12345), "1.2万");
  assert.strictEqual(fanqie.fmtReads(9999), "9999");
  assert.strictEqual(fanqie.fmtReads(0), "未知");
  assert.strictEqual(fanqie.fmtWords(10000), "1.0万");
  assert.strictEqual(fanqie.fmtWords(undefined), "未知");
  assert.strictEqual(fanqie.fmtStatus("1"), "连载中");
