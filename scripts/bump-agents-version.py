...
def collect_edits(root: Path, old: int, new: int) -> list[tuple[Path, int, str, str]]:
    ...
    # deploy-manual.md（agents_version: 33 反引号/无反引号 + 阈值形态 小于/大于 `33`）
    ...
    # UPGRADING.md 版本头 + 升级步骤行（排除含「变更」的历史条目行；阈值形态 小于/大于 `33`、不得用 v33）
    ...
    if up.exists():
        for line_no, line in enumerate(up.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            for m in re.finditer(rf"(小于|大于) `{old}`", line):
                edits.append((up, line_no, m.group(0), m.group(0).replace(str(old), str(new))))
            for m in re.finditer(rf"不得用 v{old}", line):
                edits.append((up, line_no, m.group(0), m.group(0).replace(str(old), str(new))))
    return edits
...
def main() -> int:
    ...
    # 执行替换（按行号定位，禁止全局字符串 replace——同一字面出现在应排除的历史行时会误伤；临时备份，失败回滚——覆盖 agents/setup 两类版本）
    ...
    try:
        ...
        for path in {e[0] for e in edits}:
            ...
            for p, line_no, old_s, new_s in edits:
                if p == path and 1 <= line_no <= len(lines):
                    lines[line_no - 1] = lines[line_no - 1].replace(old_s, new_s)
            new_text = "".join(lines)
            if new_text != text:
